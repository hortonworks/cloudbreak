...
FREEIPA_JAVA_OPTS="$FREEIPA_JAVA_OPTS -XX:+ExitOnOutOfMemoryError"
...
if [ "$CRYPTOSENSE_ENABLED" == "true" ]; then
  FREEIPA_JAVA_OPTS="$FREEIPA_JAVA_OPTS -Dcryptosense.agent.out=/cryptosense/logs -javaagent:/cryptosense/cs-java-tracer.jar"
fi
...
FREEIPA_JAVA_OPTS="${FREEIPA_JAVA_OPTS} ${SECURITY_OPTS} ${JACOCO_AGENT_OPTIONS}"
