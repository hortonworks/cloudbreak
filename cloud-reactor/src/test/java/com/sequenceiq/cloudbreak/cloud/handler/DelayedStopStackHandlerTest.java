...
class DelayedStopStackHandlerTest {
    ...
    @InjectMocks
    private DelayedStopStackHandler underTest;
    ...
    public void testDelayedStopMultipleInstance() throws ExecutionException, InterruptedException {
        ...
        verify(instanceConnector, times(3)).stop(eq(authenticatedContext), eq(List.of()), instanceCaptor.capture());
        ...
        verify(eventBus).notify(eq(StopInstancesResult.class.getSimpleName().toUpperCase(Locale.ROOT)), any(Event.class));
    }
    ...
    public void testDelayedStopOneInstance() {
        ...
        ArgumentCaptor<List<CloudInstance>> instanceCaptor = ArgumentCaptor.forClass(List.class);
        verify(instanceConnector, times(1)).stop(eq(authenticatedContext), eq(List.of()), instanceCaptor.capture());
        List<List<CloudInstance>> instanceCaptorAllValues = instanceCaptor.getAllValues();
    ...
    public void testExceptionThrown() {
        ...
        verifyNoInteractions(delayedExecutorService);
        verify(cloudConnector, never()).instances();
        verify(eventBus).notify(eq(StopInstancesResult.class.getSimpleName().toUpperCase(Locale.ROOT) + "_ERROR"), any(Event.class));
    }

    @Test
    public void testAccept() {
        assertEquals(DelayedStopInstancesRequest.class, underTest.type());
    }
}
