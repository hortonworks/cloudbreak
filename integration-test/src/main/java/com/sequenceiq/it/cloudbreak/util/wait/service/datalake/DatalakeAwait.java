...
@Component
public class DatalakeAwait implements Await<SdxTestDto, SdxClusterStatusResponse> {

    private static final Logger LOGGER = LoggerFactory.getLogger(DatalakeAwait.class);

    @Override
    public SdxTestDto await(SdxTestDto entity, SdxClusterStatusResponse desiredStatus, TestContext testContext, RunningParameter runningParameter,
            Duration pollingInterval, int maxRetry) {
        try {
            if (entity == null) {
                throw new RuntimeException("Datalake key has been provided but no result in resource map!");
            }
    ...
    private Result<WaitResult, Exception> waitForDatalakeStatus(ExceptionChecker<DatalakeWaitObject> statusChecker, SdxClient client,
            String name, TestContext testContext, SdxClusterStatusResponse desiredStatus, Duration pollingInterval, int maxRetry) {
