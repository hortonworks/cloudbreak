...
public class SdxInternalInstanceAwait implements Await<SdxInternalTestDto, InstanceStatus> {
    ...
    public SdxInternalTestDto await(SdxInternalTestDto entity, Map<String, InstanceStatus> desiredStatuses, TestContext testContext,
        ...
        try {
            ...
            if (isDeleted(desiredStatuses)) {
                waitForSdxInternalInstanceStatuses(new InstanceTerminationChecker<>(), name, testContext, desiredStatuses,
                        pollingInterval, maxRetry);
            } else if (isFailed(desiredStatuses)) {
                waitForSdxInternalInstanceStatuses(new InstanceFailedChecker<>(), name, testContext, desiredStatuses,
                        pollingInterval, maxRetry);
            } else {
                waitForSdxInternalInstanceStatuses(new InstanceOperationChecker<>(), name, testContext, desiredStatuses,
                        pollingInterval, maxRetry);
            }
        } catch (Exception e) {
    ...
    private Result<WaitResult, Exception> waitForSdxInternalInstanceStatuses(ExceptionChecker<InstanceWaitObject> statusChecker,
            String name, TestContext testContext, Map<String, InstanceStatus> desiredStatuses, Duration pollingInterval,
            int maxRetry) {
