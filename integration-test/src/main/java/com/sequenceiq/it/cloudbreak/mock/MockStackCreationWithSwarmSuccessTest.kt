...
class MockStackCreationWithSwarmSuccessTest : AbstractMockIntegrationTest() {
    ...
    private fun addMockEndpoints(numberOfServers: Int) {
        ...
        get(DOCKER_API_ROOT + "/containers/:container/json", "application/json", { req, res -> InspectContainerResponse("id") }, ResponseTransformer { gson().toJson(it) })
        post(DOCKER_API_ROOT + "/containers/:container/start", "application/json") { req, res -> "" }
        get(SWARM_API_ROOT + "/info", "application/json", { req, res -> Info(numberOfServers) }, ResponseTransformer { gson().toJson(it) })
