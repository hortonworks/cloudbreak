...
public class PeriscopeLoadSetupTest extends AbstractClouderaManagerTest {
    ...
    private void setUpForTenant(MockedTestContext testContext, Integer tenantIndex, ExecutorService threadPool) {
        ...
        testContext.given(credentialName, CredentialTestDto.class)
                ...
                .when(credentialTestClient.create(), RunningParameter.who(user))
                .validate();
        testContext
                .given(catalogName, ImageCatalogTestDto.class)
                .withName(catalogName)
                .when(new ImageCatalogCreateRetryAction(), RunningParameter.who(user))
                .validate();
        ...
        createCmBlueprint(testContext, bluePrintName, user);
        List<Future<String>> tasks = new ArrayList<>();
        for (int envCounter = 1; envCounter <= loadTestProperties.getNumEnvironmentsPerTenant(); envCounter++) {
            ...
            tasks.add(threadPool.submit(() -> {
                createEnvironment(testContext, envName, credentialName, catalogName, bluePrintName, imageName, user, threadPool);
                return envName;
            }));
        }
        waitingForFinishAllThread(tasks);
    ...
    private void createEnvironment(MockedTestContext testContext, String envName, String credentialName, String catalogName, String bluePrintName,
            String imageName, CloudbreakUser user, ExecutorService threadPool) {
        ...
        environmentDetails.setUser(user);
        environmentDetails.withCreateFreeIpa(Boolean.FALSE)
                .withName(envName)
                .withCredentialName(credentialName)
                .when(environmentTestClient.create(), RunningParameter.who(user))
                .await(EnvironmentStatus.AVAILABLE, RunningParameter.who(user))
        ...
        sdxInternalTestDto.withName(dataLakeName)
                ...
                .when(sdxTestClient.createInternal(), RunningParameter.who(user))
                .given(dataLakeName, SdxInternalTestDto.class)
                .await(SdxClusterStatusResponse.RUNNING, RunningParameter.who(user));
        List<Future<String>> tasks = new ArrayList<>();
        for (int dhCounter = 1; dhCounter <= loadTestProperties.getNumDataHubsPerEnvironment(); dhCounter++) {
            int dhCounterFinal = dhCounter;
            tasks.add(threadPool.submit(() -> {
                createDataHub(dhCounterFinal, testContext, envName, bluePrintName, imageName, user);
                return "";
            }));
        }
        waitingForFinishAllThread(tasks);
    ...
    private String createDataHub(int dataHubIndex, MockedTestContext testContext, String envName, String bluePrintName,
            String imageName, CloudbreakUser user) {
        ...
        distroXTestDto.withCluster(testContext.given(clusterName, DistroXClusterTestDto.class)
                ...
                .when(distroXClient.create(), RunningParameter.who(user))
                .await(STACK_AVAILABLE, RunningParameter.who(user));
        ...
        autoScaleConfigDto.when(periscopeTestClient.create(), RunningParameter.who(user));
        return dataHubName;
    }
