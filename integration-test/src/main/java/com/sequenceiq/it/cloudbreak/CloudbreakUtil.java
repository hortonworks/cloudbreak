...
public class CloudbreakUtil {
    ...
    public static void waitAndExpectClusterFailure(CloudbreakClient cloudbreakClient, Long workspaceId, String stackName,
            String desiredStatus, String keyword) {
    ...
    public static void waitAndExpectClusterFailure(CloudbreakClient cloudbreakClient, Long workspaceId, String stackName,
            Map<String, String> desiredStatuses, String keyword) {
        for (int i = 0; i < 3; i++) {
            WaitResult waitResult = waitForStatuses(cloudbreakClient, workspaceId, stackName, desiredStatuses);
            if (waitResult == WaitResult.TIMEOUT) {
                Assert.fail("Timeout happened");
    ...
    public static WaitResult waitForHostStatusStack(StackV3Endpoint stackV3Endpoint, Long workspaceId, String stackName,
            String hostGroup, String desiredStatus) {
        ...
        do {
            ...
            for (InstanceGroupResponse instanceGroupResponse : instanceGroups) {
                if (instanceGroupResponse.getGroup().equals(hostGroup)) {
                    ...
                    for (InstanceMetaDataJson instanceMetaDataJson : hostMetadataResponses) {
                        if (instanceMetaDataJson.getInstanceStatus().name().equals(desiredStatus)) {
                            found = Boolean.TRUE;
                        }
                    }
                }
    ...
    public static void checkClusterAvailability(StackV3Endpoint stackV3Endpoint, String port, Long workspaceId, String stackName,
            String ambariUser, String ambariPassowrd, boolean checkAmbari) throws IOException, URISyntaxException {
        StackResponse stackResponse = stackV3Endpoint.getByNameInWorkspace(workspaceId, stackName, new HashSet<>());
    ...
    public static void checkClusterStopped(CloudbreakClient client, String port, Long workspaceId, String stackName, String ambariUser, String ambariPassword) {
        StackResponse stackResponse = client.stackV3Endpoint().getByNameInWorkspace(workspaceId, stackName, null);
    ...
    private static WaitResult waitForStatuses(CloudbreakClient cloudbreakClient, Long workspaceId, String stackName, Map<String, String> desiredStatuses) {
        ...
        int retryCount = 0;
        do {
            LOGGER.info("Waiting for status(es) {}, stack name: {}, current status(es) {} ...", desiredStatuses, stackName, currentStatuses);
