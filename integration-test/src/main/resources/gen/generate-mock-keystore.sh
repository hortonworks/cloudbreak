...
docker run --rm -v $(pwd):/certs docker-private.infra.cloudera.com/cloudera_thirdparty/ehazlett/certm:0.2.0 -d /certs bundle generate --host localhost --host 127.0.0.1 --host *.cluster.local --host test --host mockserver.default.svc.cluster.local --host qa-mockserver.default.svc.cluster.local --host eng-mockserver.default.svc.cluster.local -o=gateway
