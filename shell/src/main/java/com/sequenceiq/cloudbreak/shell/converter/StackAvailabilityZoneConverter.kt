...
class StackAvailabilityZoneConverter : AbstractConverter<StackAvailabilityZone>() {

    @Autowired
    private val context: ShellContext? = null

    override fun supports(type: Class<*>, s: String): Boolean {
        return StackAvailabilityZone::class.java!!.isAssignableFrom(type)
    }
    ...
    override fun getAllPossibleValues(completions: MutableList<Completion>, targetType: Class<*>, existingData: String, optionContext: String, target: MethodTarget): Boolean {
        try {
            return getAllPossibleValues(completions, context!!.getAvailabilityZonesByPlatform(context.activeCloudPlatform))
        } catch (e: Exception) {
            return false
        }
