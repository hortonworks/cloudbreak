...
public class HueWorkaroundValidatorService {
    ...
    private static final int MAX_HOST_GROUP_NAME = 7;
        ...
        if (isHuePresented(hueHostGroups)) {
            if (tooLongEnpointName(endpointName)) {
                throw new BadRequestException(String.format("Your Data Hub contains Hue. Hue does not support CDP Data "
                        ...
                        + "20 characters and a hostgroup name that is shorter than 7 characters.", endpointName));
            }
        }
    }
    ...
    private void validateLimitedHostgroupNameLength(Set<String> hueHostGroups) {
        if (isHuePresented(hueHostGroups)) {
            ...
            if (!hostGroupsWhichAreLongerThanSixCharacter.isEmpty()) {
                throw new BadRequestException(String.format("Hue does not support CDP Data Hubs where hostgroup name is longer than 7 characters. "
                                + "Please retry creating the template by shortening the hostgroup name: %s  under 7 characters.",
