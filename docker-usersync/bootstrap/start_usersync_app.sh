...
echo "Starting the Usersync application..."

set -x
if [ "$SECURE_RANDOM" == "false" ]; then
  USERSYNC_JAVA_OPTS="$USERSYNC_JAVA_OPTS -Djava.security.egd=file:/dev/./urandom"
fi

eval "(java $USERSYNC_JAVA_OPTS -jar /usersync.jar) & JAVAPID=\$!; trap \"kill \$JAVAPID; wait \$JAVAPID\" SIGINT SIGTERM; wait \$JAVAPID"
