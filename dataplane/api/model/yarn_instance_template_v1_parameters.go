// Code generated by go-swagger; DO NOT EDIT.

package model

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	strfmt "github.com/go-openapi/strfmt"

	"github.com/go-openapi/swag"
)

// YarnInstanceTemplateV1Parameters yarn instance template v1 parameters
// swagger:model YarnInstanceTemplateV1Parameters
type YarnInstanceTemplateV1Parameters struct {

	// cpus
	Cpus int32 `json:"cpus,omitempty"`

	// memory
	Memory int32 `json:"memory,omitempty"`
}

// Validate validates this yarn instance template v1 parameters
func (m *YarnInstanceTemplateV1Parameters) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *YarnInstanceTemplateV1Parameters) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *YarnInstanceTemplateV1Parameters) UnmarshalBinary(b []byte) error {
	var res YarnInstanceTemplateV1Parameters
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
