...
public class AwsNetworkConnector implements NetworkConnector {
    ...
    public CreatedCloudNetwork createNetworkWithSubnets(NetworkCreationRequest networkRequest) {
        ...
        List<CloudSubnet> cloudSubnetList = getCloudSubnets(networkRequest.getCloudCredential(), new ArrayList<>(networkRequest.getSubnetCidrs()));
        String cloudFormationTemplate = createTemplate(networkRequest, cloudSubnetList);
        ...
        Set<CloudSubnet> subnets = getCreatedSubnets(output, cloudSubnetList);
    ...
    private String createTemplate(NetworkCreationRequest networkRequest, List<CloudSubnet> cloudSubnetList) {
        return awsNetworkCfTemplateProvider.provide(networkRequest.getNetworkCidr(), cloudSubnetList);
    }
    ...
    private Set<CloudSubnet> getCreatedSubnets(Map<String, String> output, List<CloudSubnet> cloudSubnetList) {
        Set<CloudSubnet> subnets = new HashSet<>();
        for (int i = 0; i < cloudSubnetList.size(); i++) {
            ...
            if (output.containsKey(expectedSubnetName)) {
                ...
                cloudSubnet.setSubnetId(output.get(expectedSubnetName));
                cloudSubnet.setCidr(cloudSubnetList.get(i).getCidr());
                cloudSubnet.setAvailabilityZone(cloudSubnetList.get(i).getAvailabilityZone());
    ...
    private List<CloudSubnet> getCloudSubnets(CloudCredential cloudCredential, List<String> subnetCidrs) {
