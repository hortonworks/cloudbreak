...
public class ClouderaManagerClusterDecommissionService implements ClusterDecomissionService {
    ...
    private ApiClient v44Client;

    private boolean bulkRemovalSupported;
    ...
    public void initApiClient() throws ClusterClientInitException {
        ...
        try {
            v44Client = clouderaManagerApiClientProvider.getV45Client(stack.getGatewayPort(), user, password, clientConfig);
        } catch (ClouderaManagerClientInitException e) {
            LOGGER.error("Client init failed for V44 client, thus bulk host removal is not supported");
        }
    }
    ...
    @Override
    public void deleteHostsFromCluster(List<InstanceMetaData> hosts) {
        if (v44Client != null) {
            clouderaManagerDecomissioner.deleteHosts(stack, hosts, v44Client);
        } else {
            LOGGER.error("Client init failed for V44 client, thus bulk host removal is not supported");
            hosts.stream().forEach(this::deleteHostFromCluster);
        }
    }
