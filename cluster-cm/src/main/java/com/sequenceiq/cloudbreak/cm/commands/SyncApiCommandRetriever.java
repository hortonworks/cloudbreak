...
public class SyncApiCommandRetriever {
     ...
     */
    public Optional<Long> getCommandId(String commandName, ClustersResourceApi api, StackView stack)
     ...
     */
    public Optional<Long> getLastFinishedCommandId(String commandName, ClustersResourceApi api, StackView stack)
            throws CloudbreakException, ApiException {
        Optional<Long> lastSyncApiCommandId = getCommandId(commandName, api, stack, true);
    ...
    private Optional<Long> getCommandId(String commandName, ClustersResourceApi api, StackView stack, boolean skipActiveRunningCommands)
            throws CloudbreakException, ApiException {
        ApiResponse<ApiCommandList> commandListResponse =
                api.listActiveCommandsWithHttpInfo(stack.getName(), null, null);
        Optional<Long> foundCommandId =
    ...
    @VisibleForTesting
    Long getCommandIdFromActiveCommands(String commandName, ApiResponse<ApiCommandList> response, boolean skipActiveRunningCommand) {
    ...
    private Long getLatestCommandId(String commandName, List<CommandResource> commandList, String path, boolean skipActiveRunningCommands) {
        if (skipActiveRunningCommands) {
            ...
            return commandList.stream()
                    ...
                    .map(CommandResource::getId)
                    .orElse(null);
        }
    }

    private Long getLatestCommandId(String commandName, List<CommandResource> commandList, String path) {
        return getLatestCommandId(commandName, commandList, path, false);
    }

    private Optional<Long> fetchCommandIdFromCommandTable(String commandName, AbstractCommandTableResource commandTable,
    ...
    private CommandResource convertApiCommandToCommandResource(ApiCommand apiCommand) {
        ...
        command.setStart(apiCommand.getStartTime().toEpochSecond());
