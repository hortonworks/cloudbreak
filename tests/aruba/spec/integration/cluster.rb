...
RSpec.describe 'Cluster test cases', :type => :aruba do
  ...
  it "Cluster - Create - Without Name" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.create.name("").cli_input_json(@cli_input_json).build 
  ...
  it "Cluster - Create - Without password" do 
    skip("BUG-94445")
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.create.name("asdfg").cli_input_json("../../templates/template-wo-pwd.json").build
      expect(result.exit_status).to eql 1
  ...
  it "Cluster - Create - Without password by parameter" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.create.name("asdfg").cli_input_json("../../templates/template-wo-pwd.json").input_json_param_password("pass123").build 
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("stack created")          
    end
  end      
  ...
  it "Cluster - Describe Success" do
    result = cb.cluster.describe.name("openstack-cluster").build 
    expect(result.exit_status).to eql 0
    expect(JSON.parse(result.stdout)["name"]).to eql "openstack-cluster"    
  end

  it "Cluster - Describe Failure" do
    result = cb.cluster.describe.name("az404").build   
    expect(result.exit_status).to eql 1
   ...
   it "Cluster - Stop - Success" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.stop.name("asdfg").build 
      expect(result.exit_status).to eql 0
   ...
   it "Cluster - Stop - Failure" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.stop.name("azstatus").build
      expect(result.exit_status).to eql 1
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.start.name("asdfg").build
      expect(result.exit_status).to eql 0
   ...
   it "Cluster - Start - Failure" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.start.name("azstatus").build
      expect(result.exit_status).to eql 1
      expect(result.stderr).to include("Stack 'azstatus' not found")          
    end   
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.sync.name("asdfg").build
      expect(result.exit_status).to eql 0
   ...
   it "Cluster - Sync - Failure" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.sync.name("azstatus").build
      expect(result.exit_status).to eql 1
      expect(result.stderr).to include("Stack 'azstatus' not found")          
    end   
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.repair.name("asdfg").build
      expect(result.exit_status).to eql 0
   ...
   xit "Cluster - Repair - Failure" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.repair.name("azstatus").build 
      expect(result.exit_status).to eql 1
      expect(result.stderr).to include("Stack 'azstatus' not found")          
    end   
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.scale.name("climock").group_name("hgroup").desired_node_count(3).build             
      expect(result.exit_status).to eql 0
   ...
   it "Cluster - Scale - Failure" do 
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.scale.name("azstatus").group_name("hgroup").desired_node_count(3).build   
      expect(result.exit_status).to eql 1
      expect(result.stderr).to include("Stack 'azstatus' not found")          
    end                   
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.reinstall.name("asdfg").cli_input_json(@cli_input_json).blueprint_name("asdfg").build
      expect(result.exit_status).to eql 0
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.aws.new_network.build
      expect(result.exit_status).to eql 0        
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.aws.existing_network.build
      expect(result.exit_status).to eql 0        
    end
  end 
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.aws.existing_subnet.build
      expect(result.exit_status).to eql 0        
    end
  end     
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.azure.new_network.build  
      expect(result.exit_status).to eql 0        
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.azure.existing_subnet.build 
      expect(result.exit_status).to eql 0        
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.gcp.new_network.build
      expect(result.exit_status).to eql 0        
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.gcp.existing_network.build 
      expect(result.exit_status).to eql 0        
    end
  end 
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.gcp.existing_subnet.build
      expect(result.exit_status).to eql 0        
    end
  end 
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.openstack.new_network.build 
      expect(result.exit_status).to eql 0        
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.openstack.existing_network.build 
      expect(result.exit_status).to eql 0        
    end
  end 
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.cluster.generate_template.openstack.existing_subnet.build
      expect(result.exit_status).to eql 0       
    end
  end           
end  
