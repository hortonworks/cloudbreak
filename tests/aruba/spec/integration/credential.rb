...
RSpec.describe 'Credential test cases', :type => :aruba do
  ...
  it "Credential - Create - Openstack V2 Credential" do 
    with_environment 'DEBUG' => '1' do
      result = cb.credential.create.openstack.keystone_v2.name("cli-os-cred").tenant_user(@os_tenant_user).
        tenant_password(@os_tenant_pwd).tenant_name(@os_tenant_name).endpoint(@os_tenant_endpoint).build
  ...
  it "Credential - Create - AWS Role based" do 
    with_environment 'DEBUG' => '1' do
      result = cb.credential.create.aws.role_based.name("cli-aws-role").role_arn(@aws_role_arn).build
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("credential created")    
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      result = cb.credential.create.aws.key_based.name("cli-aws-key").access_key(@aws_access_key).secret_key(@aws_secret_key).build  
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("credential created")    
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      ...
      tenant_id(@az_tenant_id).app_id(@az_app_id).app_password(@az_app_pwd).build  
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("credential created")    
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      ...
      service_account_private_key_file("temp.p12").build
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("credential created")    
    end
  end
    ...
    with_environment 'DEBUG' => '1' do
      ...
      result = cb.credential.create.from_file.cli_input_json("mock.json").build
      expect(result.exit_status).to eql 0
      expect(result.stderr).to include("credential created")    
    end
  end
  ...
  it "Credential - Describe" do
    result = cb.credential.describe.name("mockcred").build
    expect(result.exit_status).to eql 0
