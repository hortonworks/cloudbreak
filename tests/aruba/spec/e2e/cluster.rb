...
RSpec.describe 'Custer operation test cases', :type => :aruba do
  ...
  xit "Start cluster" do
    skip_if(@os_cluster_name, "STOPPED", "Test is skipped because of cluster is not STOPPED")    
    result = cb.cluster.start.name(@os_cluster_name).build          
    expect(result.exit_status).to eql 0
    expect(wait_for_status(cb, @os_cluster_name, "AVAILABLE")).to eq("AVAILABLE")      
  end

  xit "Upscale cluster" do
  ...
  xit "List clusters - checking created cluster" do
    expect(cluster_exists(cb, @os_cluster_name)).to be_truthy
  end

  xit "Change ambari password" do    
    result = cb.cluster.change_ambari_password.name(@os_cluster_name).ambari_user(@ambari_user).old_password(@ambari_passwors).new_password("admintemp").build
    expect(result.exit_status).to eql 0      
  end

  xit "Generate reinstall template" do 
     result = cb.cluster.generate_reinstall_template.name(@os_cluster_name).blueprint_name(@blueprint_name).build
     expect(result.exit_status).to eql 0  
     expect(JSON.parse(result.stdout)["blueprintName"]).to eq(@blueprint_name.gsub("'",""))    
  end           
end
