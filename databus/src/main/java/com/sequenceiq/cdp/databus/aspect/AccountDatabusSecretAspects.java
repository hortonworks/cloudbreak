package com.sequenceiq.cdp.databus.aspect;
...
@Component
@Aspect
public class AccountDatabusSecretAspects {

    private static final Logger LOGGER = LoggerFactory.getLogger(AccountDatabusSecretAspects.class);

    @Inject
    private SecretAspectService secretAspectService;

    @Pointcut("execution(public * com.sequenceiq.cdp.databus.repository..*.save(..)) ")
    public void onRepositorySave() {
    }

    @Pointcut("execution(public void com.sequenceiq.cdp.databus.repository..*.delete(..)) ")
    public void onRepositoryDelete() {
    }

    @Pointcut("execution(public * com.sequenceiq.cdp.databus.repository..*.saveAll(..)) ")
    public void onRepositorySaveAll() {
    }

    @Pointcut("execution(public void com.sequenceiq.cdp.databus.repository..*.deleteAll(..)) ")
