...
CREATE UNIQUE INDEX unq_index_sdxcluster_accid_crn_detached_deleted_is_null
    ON sdxcluster (accountid, crn, deleted, detached)
    WHERE deleted IS NULL;

CREATE UNIQUE INDEX unq_index_sdxcluster_accid_envcrn_detached_deleted_is_not_null
    ON sdxcluster (accountid, envCrn, deleted, detached)
    WHERE deleted IS NOT NULL;

CREATE UNIQUE INDEX unq_index_sdxcluster_accid_crn_detached_deleted_is_not_null
    ON sdxcluster (accountid, crn, deleted, detached)
    WHERE deleted IS NOT NULL;
