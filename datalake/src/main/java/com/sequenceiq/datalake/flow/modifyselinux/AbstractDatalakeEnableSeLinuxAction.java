...
abstract class AbstractDatalakeEnableSeLinuxAction<P extends ResourceCrnPayload>
        extends AbstractAction<DatalakeModifySeLinuxState, DatalakeModifySeLinuxStateSelectors, CommonContext, P> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractDatalakeEnableSeLinuxAction.class);

    protected AbstractDatalakeEnableSeLinuxAction(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected CommonContext createFlowContext(FlowParameters flowParameters,
            StateContext<DatalakeModifySeLinuxState, DatalakeModifySeLinuxStateSelectors> stateContext,
            P payload) {
