...
public class SdxResizeValidator {
    ...
    public void validateDatabaseTypeForResize(SdxDatabase sdxDatabase, CloudPlatform cloudPlatform) {
        if (CloudPlatform.AZURE.equals(cloudPlatform) && hasExternalDatabase(sdxDatabase) && flexibleServerUpgradeEnabled() && isSingleServer(sdxDatabase)) {
    ...
    private boolean flexibleServerUpgradeEnabled() {
        return entitlementService.isAzureDatabaseFlexibleServerUpgradeEnabled(ThreadBasedUserCrnProvider.getAccountId());
    }
