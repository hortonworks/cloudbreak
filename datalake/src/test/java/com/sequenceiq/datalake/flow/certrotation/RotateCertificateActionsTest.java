...
class RotateCertificateActionsTest {
    ...
    @InjectMocks
    private RotateCertificateActions underTest;
    ...
    void stackUpgradeHappyPath() {
        actionPayload = new RotateCertificateStackEvent(ACTION_PAYLOAD_SELECTOR, SDX_ID, USER_ID);
        setupContextWithPayload();
        testUpgradeActionHappyPath(underTest::rotateCertificateStack);
        RotateCertificateStackRequest payload = (RotateCertificateStackRequest) payloadArgumentCaptor.getValue();
        assertThat(selectorArgumentCaptor.getValue()).isEqualTo(RotateCertificateStackRequest.class.getSimpleName());
    ...
    void finishedHappyPath() {
        actionPayload = new RotateCertificateSuccessEvent(SDX_ID, USER_ID);
        setupContextWithPayload();
        ...
        verify(metricService).incrementMetricCounter(MetricType.ROTATE_DATABASE_CERTIFICATE_FINISHED, sdxCluster);
        verify(sdxStatusService).setStatusForDatalakeAndNotify(
                DatalakeStatusEnum.RUNNING, "Rotate Certificate completed successfully", SDX_ID);
        RotateCertificateSuccessEvent payload = (RotateCertificateSuccessEvent) payloadArgumentCaptor.getValue();
        assertThat(selectorArgumentCaptor.getValue()).isEqualTo(ROTATE_CERTIFICATE_FINALIZED_EVENT.event());
        assertThat(payload.getResourceId()).isEqualTo(SDX_ID);
        assertThat(payload.getUserId()).isEqualTo(USER_ID);
    }
    ...
    void failureAction() {
        ...
        actionPayload = new RotateCertificateFailedEvent(SDX_ID, USER_ID, failureException);
        setupContextWithPayload();
        testUpgradeActionHappyPath(underTest::failedAction);
        RotateCertificateFailedEvent payload = (RotateCertificateFailedEvent) payloadArgumentCaptor.getValue();
        assertThat(selectorArgumentCaptor.getValue()).isEqualTo(ROTATE_CERTIFICATE_FAILED_HANDLED_EVENT.event());
        assertThat(payload.getResourceId()).isEqualTo(SDX_ID);
        assertThat(payload.getUserId()).isEqualTo(USER_ID);
        verify(sdxStatusService).setStatusForDatalakeAndNotify(DATALAKE_DATABASE_CERTIFICATE_ROTATION_FAILED, MESSAGE, SDX_ID);
        verify(metricService).incrementMetricCounter(eq(ROTATE_DATABASE_CERTIFICATE_FAILED), (SdxCluster) any());
    }
