...
<span ng-repeat="actual in activeClusterEvents |orderBy:eventTimestamp|filter:logFilterFunction"><span class="{{$root.config.EVENT_CLASS[actual .eventType]}}" >{{actual.customTimeStamp}} {{actual.stackName}} is {{$root.config.EVENT_TYPE[actual .eventType]}}: {{actual.eventMessage}}</span><br/></span>
                                                </pre>
                                            </div><!-- .col-sm-12  -->
                                        </div><!-- .form-group -->
