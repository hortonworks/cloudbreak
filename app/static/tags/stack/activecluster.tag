...
<span ng-repeat="actual in activeClusterEvents |orderBy:eventTimestamp"><span class="{{$root.config.EVENT_CLASS[actual .eventType]}}" >{{actual.eventTimestamp.replace("T", " ").replace("Z", " ").substring(0, 19)}} {{actual.stackName}} is {{$root.config.EVENT_TYPE[actual .eventType]}}: {{actual.eventMessage}}</span><br/></span>
                                                </pre>
                                            </div><!-- .col-sm-12  -->
                                        </div><!-- .form-group -->
