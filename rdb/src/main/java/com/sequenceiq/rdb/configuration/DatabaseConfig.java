...
import com.sequenceiq.rdb.service.ha.RdbNodeConfig;
import com.sequenceiq.rdb.util.DatabaseUtil;
...
public class DatabaseConfig {
    ...
    @Value("${rdb.db.env.user:}")
    private String dbUser;

    @Value("${rdb.db.env.pass:}")
    private String dbPassword;

    @Value("${rdb.db.env.db:}")
    private String dbName;

    @Value("${rdb.db.env.poolsize:10}")
    private int poolSize;

    @Value("${rdb.db.env.connectiontimeout:30}")
    private long connectionTimeout;

    @Value("${rdb.db.env.minidle:2}")
    private int minimumIdle;

    @Value("${rdb.db.env.idletimeout:10}")
    private long idleTimeout;

    @Value("${rdb.db.env.schema:" + DatabaseUtil.DEFAULT_SCHEMA_NAME + '}')
    private String dbSchemaName;

    @Value("${rdb.db.env.ssl:}")
    private boolean ssl;

    @Value("#{'${rdb.cert.dir:}/${rdb.db.env.cert.file:}'}")
    private String certFile;

    @Value("${rdb.hbm2ddl.strategy:validate}")
    private String hbm2ddlStrategy;

    @Value("${rdb.hibernate.debug:false}")
    ...
    @Inject
    private RdbNodeConfig rdbNodeConfig;
    ...
    public DataSource dataSource() throws SQLException {
        ...
        if (rdbNodeConfig.isNodeIdSpecified()) {
            config.addDataSourceProperty("ApplicationName", rdbNodeConfig.getId());
        }
    ...
    public EntityManagerFactory entityManagerFactory() throws SQLException {
        ...
        entityManagerFactory.setPackagesToScan("com.sequenceiq.rdb.domain");
