...
class CcmV2RetryingClientTest {
    ...
    void testgetOrCreateInvertingProxyWhenException() {
        assertThatThrownBy(() -> underTest.awaitReadyInvertingProxyForAccount(TEST_REQUEST_ID, TEST_ACCOUNT_ID))
                .isInstanceOf(CcmV2Exception.class);
    }

    @Test
    void testRegisterAgentWhenException() {
        assertThatThrownBy(() -> underTest.registerInvertingProxyAgent(TEST_REQUEST_ID, TEST_AGENT_CRN,
                        Optional.of(TEST_ENVIRONMENT_CRN), DOMAIN, KEY_ID, Optional.of(HMAC)))
                .isInstanceOf(CcmV2Exception.class);
    }

    @Test
    void testUnRegisterAgentWhenException() {
        assertThatThrownBy(() -> underTest.deregisterInvertingProxyAgent(TEST_REQUEST_ID, TEST_AGENT_CRN))
                    .isInstanceOf(CcmV2Exception.class);
    }

    @Test
    void testListAgentsWhenException() {
        assertThatThrownBy(() -> underTest.listInvertingProxyAgents(TEST_REQUEST_ID, TEST_ACCOUNT_ID, Optional.of(TEST_ENVIRONMENT_CRN)))
                    .isInstanceOf(CcmV2Exception.class);
    }
}
