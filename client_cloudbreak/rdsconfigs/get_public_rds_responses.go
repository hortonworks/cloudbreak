package rdsconfigs

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-swagger/go-swagger/client"
	"github.com/go-swagger/go-swagger/httpkit"

	strfmt "github.com/go-swagger/go-swagger/strfmt"

	"github.com/hortonworks/hdc-cli/models_cloudbreak"
)

// GetPublicRdsReader is a Reader for the GetPublicRds structure.
type GetPublicRdsReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the recieved o.
func (o *GetPublicRdsReader) ReadResponse(response client.Response, consumer httpkit.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetPublicRdsOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, client.NewAPIError("unknown error", response, response.Code())
	}
}

// NewGetPublicRdsOK creates a GetPublicRdsOK with default headers values
func NewGetPublicRdsOK() *GetPublicRdsOK {
	return &GetPublicRdsOK{}
}

/*GetPublicRdsOK handles this case with default header values.

successful operation
*/
type GetPublicRdsOK struct {
	Payload *models_cloudbreak.RDSConfigResponse
}

func (o *GetPublicRdsOK) Error() string {
	return fmt.Sprintf("[GET /rdsconfigs/account/{name}][%d] getPublicRdsOK  %+v", 200, o.Payload)
}

func (o *GetPublicRdsOK) readResponse(response client.Response, consumer httpkit.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models_cloudbreak.RDSConfigResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
