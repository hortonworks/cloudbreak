package networks

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-swagger/go-swagger/client"
	"github.com/go-swagger/go-swagger/httpkit"

	strfmt "github.com/go-swagger/go-swagger/strfmt"

	"github.com/hortonworks/hdc-cli/models_cloudbreak"
)

// GetPublicsNetworkReader is a Reader for the GetPublicsNetwork structure.
type GetPublicsNetworkReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the recieved o.
func (o *GetPublicsNetworkReader) ReadResponse(response client.Response, consumer httpkit.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetPublicsNetworkOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, client.NewAPIError("unknown error", response, response.Code())
	}
}

// NewGetPublicsNetworkOK creates a GetPublicsNetworkOK with default headers values
func NewGetPublicsNetworkOK() *GetPublicsNetworkOK {
	return &GetPublicsNetworkOK{}
}

/*GetPublicsNetworkOK handles this case with default header values.

successful operation
*/
type GetPublicsNetworkOK struct {
	Payload []*models_cloudbreak.NetworkResponse
}

func (o *GetPublicsNetworkOK) Error() string {
	return fmt.Sprintf("[GET /networks/account][%d] getPublicsNetworkOK  %+v", 200, o.Payload)
}

func (o *GetPublicsNetworkOK) readResponse(response client.Response, consumer httpkit.Consumer, formats strfmt.Registry) error {

	// response payload
	if err := consumer.Consume(response.Body(), &o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
