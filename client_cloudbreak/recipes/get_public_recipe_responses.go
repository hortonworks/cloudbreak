package recipes

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"fmt"
	"io"

	"github.com/go-swagger/go-swagger/client"
	"github.com/go-swagger/go-swagger/httpkit"

	strfmt "github.com/go-swagger/go-swagger/strfmt"

	"github.com/hortonworks/hdc-cli/models_cloudbreak"
)

// GetPublicRecipeReader is a Reader for the GetPublicRecipe structure.
type GetPublicRecipeReader struct {
	formats strfmt.Registry
}

// ReadResponse reads a server response into the recieved o.
func (o *GetPublicRecipeReader) ReadResponse(response client.Response, consumer httpkit.Consumer) (interface{}, error) {
	switch response.Code() {

	case 200:
		result := NewGetPublicRecipeOK()
		if err := result.readResponse(response, consumer, o.formats); err != nil {
			return nil, err
		}
		return result, nil

	default:
		return nil, client.NewAPIError("unknown error", response, response.Code())
	}
}

// NewGetPublicRecipeOK creates a GetPublicRecipeOK with default headers values
func NewGetPublicRecipeOK() *GetPublicRecipeOK {
	return &GetPublicRecipeOK{}
}

/*GetPublicRecipeOK handles this case with default header values.

successful operation
*/
type GetPublicRecipeOK struct {
	Payload *models_cloudbreak.RecipeResponse
}

func (o *GetPublicRecipeOK) Error() string {
	return fmt.Sprintf("[GET /recipes/account/{name}][%d] getPublicRecipeOK  %+v", 200, o.Payload)
}

func (o *GetPublicRecipeOK) readResponse(response client.Response, consumer httpkit.Consumer, formats strfmt.Registry) error {

	o.Payload = new(models_cloudbreak.RecipeResponse)

	// response payload
	if err := consumer.Consume(response.Body(), o.Payload); err != nil && err != io.EOF {
		return err
	}

	return nil
}
