...
public class ClusterComponentConfigProvider {
    ...
    public void cleanUpDetachedEntries(Collection<Long> clusterIds) {
        LOGGER.debug("About to delete detached {} entries.", ClusterComponentHistoryView.class.getSimpleName());
        long deleted = clusterComponentHistoryRepository.deleteByClusterIdIsNullOrClusterIdIn(clusterIds);
        LOGGER.debug("Cleanung up detached {} entries has finished ({} entry has been removed).", ClusterComponentHistoryView.class.getSimpleName(), deleted);
    }
