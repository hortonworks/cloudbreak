...
class CustomConfigsServiceTest {
    ...
    @BeforeEach
    void setUp() {
        underTest = new CustomConfigsService(customConfigsRepository, validator, regionAwareCrnGenerator);
    }
    ...
    void testIfCustomConfigsAreRetrievedByCrn() {
        when(customConfigsRepository.findCustomConfigsByResourceCrn(TEST_CRN)).thenReturn(Optional.of(customConfigs));
        ...
        verify(customConfigsRepository).findCustomConfigsByResourceCrn(TEST_CRN);
    }
    ...
    void testIfCustomConfigsAreRetrievedByName() {
        when(customConfigsRepository.findCustomConfigsByNameAndAccountId(TEST_NAME, TEST_ACCOUNT_ID)).thenReturn(Optional.of(customConfigs));
        CustomConfigs returnedValue = underTest.getByName(TEST_NAME, TEST_ACCOUNT_ID);
        assertEquals(customConfigs, returnedValue);
        verify(customConfigsRepository).findCustomConfigsByNameAndAccountId(TEST_NAME, TEST_ACCOUNT_ID);
    }
