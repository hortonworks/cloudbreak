...
public interface CustomConfigsRepository extends JpaRepository<CustomConfigs, Long> {
    ...
    Optional<CustomConfigs> findCustomConfigsByNameAndAccountId(String customConfigsName, String accountId);

    @Query("SELECT c FROM CustomConfigs c LEFT JOIN FETCH c.configs WHERE c.resourceCrn=?1")
    Optional<CustomConfigs> findCustomConfigsByResourceCrn(String resourceCrn);
    ...
    @Query("SELECT DISTINCT c FROM CustomConfigs c LEFT JOIN FETCH c.configs WHERE c.account=?1")
    List<CustomConfigs> findCustomConfigsByAccountId(String accountId);
