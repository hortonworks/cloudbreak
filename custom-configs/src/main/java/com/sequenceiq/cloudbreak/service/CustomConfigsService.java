...
public class CustomConfigsService implements ResourcePropertyProvider {
    ...
    public CustomConfigs getByCrn(String crn) {
        Optional<CustomConfigs> customConfigsByCrn = customConfigsRepository.findCustomConfigsByResourceCrn(crn);
    ...
    private void validate(CustomConfigs customConfigs) {
        try {
            validator.validateIfAccountIsEntitled();
    ...
    public CustomConfigs getByName(String name, String accountId) {
        Optional<CustomConfigs> customConfigsByName = customConfigsRepository.findCustomConfigsByNameAndAccountId(name, accountId);
    ...
    public CustomConfigs deleteByCrn(String crn) {
        ...
        // since we're outside core module and core module has dependency on custom configs,
        // Adding core module as a dependency to this one would cause circular dependency.
        customConfigsRepository.deleteById(customConfigsByCrn.get().getId());
        return customConfigsByCrn.get();
    }

    public CustomConfigs deleteByName(String name, String accountId) {
        Optional<CustomConfigs> customConfigsByName = Optional.of(customConfigsRepository.findCustomConfigsByNameAndAccountId(name, accountId)
                .orElseThrow(NotFoundException.notFound("CustomConfigs", name)));
        customConfigsRepository.deleteById(customConfigsByName.get().getId());
        return customConfigsByName.get();
    }
