...
public class CaasClient {
    ...
    public String getAccessToken(TokenRequest refreshToken) {
        try (Response resp = getCaasWebTarget().path("/oidc/token")
                .request().post(entity(refreshToken, MediaType.APPLICATION_JSON_TYPE))) {
            return resp.readEntity(TokenResponse.class).getAccessToken();
        }
    }
