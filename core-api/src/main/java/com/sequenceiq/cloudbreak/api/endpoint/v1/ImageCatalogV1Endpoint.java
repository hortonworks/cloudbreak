...
public interface ImageCatalogV1Endpoint {
    ...
    @GET
    @Path("account/upgrade/{clusterName}/{name}")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = ImageCatalogOpDescription.GET_IMAGES_BY_CLUSTER_NAME_AND_CUSTOM_IMAGE_CATALOG, produces = ContentType.JSON,
            notes = Notes.IMAGE_CATALOG_NOTES, nickname = "getImagesByClusterNameAndCustomImageCatalog")
    ImagesResponse getImagesFromCustomImageCatalogByCluster(@PathParam("name") String name, @PathParam("clusterName") String clusterName) throws Exception;

    @GET
    @Path("account/upgrade/{clusterName}")
    @Produces(MediaType.APPLICATION_JSON)
    @ApiOperation(value = ImageCatalogOpDescription.GET_IMAGES_BY_CLUSTER_NAME, produces = ContentType.JSON,
            notes = Notes.IMAGE_CATALOG_NOTES, nickname = "getImagesByClusterNameAndDefaultImageCatalog")
    ImagesResponse getImagesFromDefaultImageCatalogByCluster(@PathParam("clusterName") String clusterName) throws Exception;
