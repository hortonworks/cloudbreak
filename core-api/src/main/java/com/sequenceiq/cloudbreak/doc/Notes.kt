...
object Notes {
    ...
    val BLUEPRINT_NOTES = "Ambari Blueprints are a declarative "
    ...
    val TEMPLATE_NOTES = "A template gives developers and systems administrators "
    ...
    val CONSTRAINT_NOTES = "A constraint template tells Cloudbreak the resource constraints "
    ...
    val CREDENTIAL_NOTES = "Cloudbreak is launching Hadoop clusters on the user's behalf - "
    ...
    val STACK_NOTES = "Stacks are template instances - a running cloud infrastructure "
    ...
    val CLUSTER_NOTES = "Clusters are materialised Hadoop services on a given infrastructure. "
    ...
    val RECIPE_NOTES = "Recipes are basically script extensions to a cluster that run on a set of nodes" + " before or after the Ambari cluster installation."
    val SSSDCONFIG_NOTES = "SSSD configs are defining external user database configuration for Hadoop containers."
    val USAGE_NOTES = "Cloudbreak gives you an up to date overview of cluster usage based " + "on different filtering criteria (start/end date, users, providers, region, etc)"
    val EVENT_NOTES = "Events are used to track stack creation initiated by cloudbreak users. " + "Events are generated by the backend when resources requested by the user become available or unavailable"
    val NETWORK_NOTES = "Provider specific network settings could be configured by using Network resources."
    val SECURITY_GROUP_NOTES = "Different inbound security rules(group) could be configured by using SecurityGroup resources " + "and a group could be assigned to any Stack(cluster)."
    val ACCOUNT_PREFERENCES_NOTES = "Account related preferences that could be managed by the account admins and different " + "restrictions could be added to Cloudbreak resources."

    val USER_NOTES = "Users can be invited under an account by the administrator, and all resources " + "(e.g. resources, networks, blueprints, credentials, clusters) can be shared across account users"
    val TOPOLOGY_NOTES = "A topology gives system administrators an easy way to associate compute nodes with data centers and racks."
}
