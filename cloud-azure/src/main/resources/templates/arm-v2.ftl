            ...
            <#if !existingVPC>
            {
             ...
             <#list groups[instanceGroup] as instance>
                 ...
                 <#if !noPublicIp>
                 {
                   ...
                   <#if (loadBalancerMapping[instance.groupName]?? && (loadBalancers?filter(loadBalancer -> loadBalancer.sku == "STANDARD")?size > 0)) || (instance.instance.availabilityZone?? && instance.instance.availabilityZone?has_content)>
                     "sku": {
                         "name": "Standard",
                         "tier": "Regional"
                     },
