...
public class AzureExistingPrivateDnsZoneValidatorService {
    ...
    public ValidationResult.ValidationResultBuilder validate(AzureClient azureClient, String networkResourceGroupName,
        ...
        serviceToPrivateDnsZoneId.forEach((service, privateDnsZoneId) -> {
            try {
                azurePrivateDnsZoneValidatorService.existingPrivateDnsZoneNameIsSupported(service, privateDnsZoneId, resultBuilder);
                azurePrivateDnsZoneValidatorService.privateDnsZoneExists(azureClient, privateDnsZoneId, resultBuilder);
                azurePrivateDnsZoneValidatorService.privateDnsZoneConnectedToNetwork(azureClient, networkResourceGroupName, networkName, privateDnsZoneId,
                        resultBuilder);
