...
class AzureLoadBalancerTagUpdateStrategyTest {
    ...
    @InjectMocks
    private AzureLoadBalancerTagUpdateStrategy underTest;
    ...
    @Test
    void testUpdateTagsAzureLoadBalancer() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_LOAD_BALANCER, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);

        underTest.updateTags(authenticatedContext, cloudResource, USER_DEFINED_TAGS);

        verify(azureClient).updateLoadBalancerTags(RESOURCE_REFERENCE, USER_DEFINED_TAGS);
    }

    @Test
    void testUpdateTagsAzureLoadBalancerWithoutNewTags() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_LOAD_BALANCER, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);
        when(azureClient.getLoadBalancerTags(RESOURCE_REFERENCE)).thenReturn(USER_DEFINED_TAGS);

        underTest.updateTags(authenticatedContext, cloudResource, USER_DEFINED_TAGS);

        verify(azureClient, times(0)).updateLoadBalancerTags(RESOURCE_REFERENCE, USER_DEFINED_TAGS);
    }

    @Test
    void testUpdateTagsAzureLoadBalancerSkippedWhenReferenceIsNull() {
