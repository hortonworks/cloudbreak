...
public class AzureTemplateBuilderDbTest {
    ...
    @InjectMocks
    private AzureTemplateBuilder underTest;
    ...
    private void buildTestWhenUseSslEnforcementInternal(String templatePath, boolean useSslEnforcement) {
        ReflectionTestUtils.setField(underTest, "armDatabaseTemplatePath", templatePath);
        String template = Optional.ofNullable(factoryBean.getObject())
                .map(config -> {
                    try {
        ...
        DatabaseStack databaseStack = createDatabaseStack(useSslEnforcement, template);
