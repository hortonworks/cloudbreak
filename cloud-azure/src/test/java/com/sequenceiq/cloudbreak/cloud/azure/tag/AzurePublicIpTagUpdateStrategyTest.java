...
class AzurePublicIpTagUpdateStrategyTest {
    ...
    @Test
    void testDeleteTagsAzurePublicIp() {
        ...
        when(azureClient.getPublicIpTags(RESOURCE_REFERENCE)).thenReturn(EXISTING_TAGS);

        underTest.deleteTags(authenticatedContext, cloudResource, Set.of("existingKey"));

        verify(azureClient).updatePublicIpTags(RESOURCE_REFERENCE, Map.of());
    }
    ...
    void testDeleteTagsSkipWhenKeyNotPresent() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_PUBLIC_IP, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);
        when(azureClient.getPublicIpTags(RESOURCE_REFERENCE)).thenReturn(Map.of("otherKey", "otherValue"));

        underTest.deleteTags(authenticatedContext, cloudResource, Set.of("existingKey"));

        verify(azureClient, times(0)).updatePublicIpTags(RESOURCE_REFERENCE, Map.of());
    }
    ...
    @Test
    void testUpdateTagsAzurePublicIpSkippedWhenReferenceIsNull() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_PUBLIC_IP, null, null);
    ...
    @Test
    void testDeleteTagsAzurePublicIpSkippedWhenReferenceIsNull() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_PUBLIC_IP, null, null);
