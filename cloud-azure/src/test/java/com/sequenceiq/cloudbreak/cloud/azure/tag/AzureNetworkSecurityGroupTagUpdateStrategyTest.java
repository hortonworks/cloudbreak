...
class AzureNetworkSecurityGroupTagUpdateStrategyTest {
    ...
    @InjectMocks
    private AzureNetworkSecurityGroupTagUpdateStrategy underTest;
    ...
    @Test
    void testDeleteTagsAzureSecurityGroup() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_SECURITY_GROUP, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);
        when(azureClient.getNetworkSecurityGroupTags(RESOURCE_REFERENCE)).thenReturn(EXISTING_TAGS);

        underTest.deleteTags(authenticatedContext, cloudResource, Set.of("existingKey"));

        verify(azureClient).updateNetworkSecurityGroupTags(RESOURCE_REFERENCE, Map.of());
    }
    ...
    void testDeleteTagsSkipWhenKeyNotPresent() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_SECURITY_GROUP, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);
        when(azureClient.getNetworkSecurityGroupTags(RESOURCE_REFERENCE)).thenReturn(Map.of("otherKey", "otherValue"));

        underTest.deleteTags(authenticatedContext, cloudResource, Set.of("existingKey"));

        verify(azureClient, times(0)).updateNetworkSecurityGroupTags(RESOURCE_REFERENCE, Map.of());
    }

    @Test
    void testUpdateTagsAzureSecurityGroup() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_SECURITY_GROUP, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);

        underTest.updateTags(authenticatedContext, cloudResource, USER_DEFINED_TAGS);

        verify(azureClient).updateNetworkSecurityGroupTags(RESOURCE_REFERENCE, USER_DEFINED_TAGS);
    }

    @Test
    void testUpdateTagsAzureSecurityGroupWithoutNewTags() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_SECURITY_GROUP, null, RESOURCE_REFERENCE);
        when(azureClientService.getClient(cloudContext, cloudCredential)).thenReturn(azureClient);
        when(azureClient.getNetworkSecurityGroupTags(RESOURCE_REFERENCE)).thenReturn(USER_DEFINED_TAGS);
    ...
    @Test
    void testDeleteTagsAzureSecurityGroupSkippedWhenReferenceIsNull() {
        CloudResource cloudResource = buildResource(ResourceType.AZURE_SECURITY_GROUP, null, null);
