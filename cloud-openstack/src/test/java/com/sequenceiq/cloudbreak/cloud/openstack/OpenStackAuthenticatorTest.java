...
class OpenStackAuthenticatorTest {
    ...
    @Test
    void authenticateShouldRegisterWhenJumpgateEnvCrnSetAndNotRegistered() {
        ...
        when(keystoneCredentialView.getCredentialCrn()).thenReturn(CREDENTIAL_CRN);
        when(clusterProxyService.isRegistered(CREDENTIAL_CRN)).thenReturn(false);
        ...
        verify(clusterProxyService).registerServices(keystoneCredentialView);
    ...
    void authenticateShouldSkipRegistrationWhenAlreadyRegistered() {
        ...
        when(keystoneCredentialView.getJumpgateEnvironmentCrn()).thenReturn(JUMPGATE_ENV_CRN);
        when(keystoneCredentialView.getCredentialCrn()).thenReturn(CREDENTIAL_CRN);
        when(clusterProxyService.isRegistered(CREDENTIAL_CRN)).thenReturn(true);
        when(openStackClient.createAuthenticatedContext(cloudContext, cloudCredential)).thenReturn(authenticatedContext);

        underTest.authenticate(cloudContext, cloudCredential);

        verify(clusterProxyService, never()).registerServices(any());
        verify(openStackClient).createAuthenticatedContext(cloudContext, cloudCredential);
    }

    @Test
    void authenticateShouldSkipRegistrationWhenNoJumpgateEnvCrn() {
        CloudContext cloudContext = createCloudContext();
        CloudCredential cloudCredential = createCloudCredential();
        ...
        verify(clusterProxyService, never()).isRegistered(anyString());
        verify(clusterProxyService, never()).registerServices(any());
    }
    ...
    private CloudCredential createCloudCredential() {
        return new CloudCredential(CREDENTIAL_CRN, CREDENTIAL_NAME, null, ACCOUNT_ID);
    }
