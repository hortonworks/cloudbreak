...
public class OpenStackInstanceBuilder extends AbstractOpenStackComputeResourceBuilder {
    ...
    public List<CloudResource> build(OpenStackContext context, CloudInstance instance, long privateId, AuthenticatedContext auth, Group group,
        ...
        try {
            ...
            Optional<? extends Server> existingServer = osClient.compute().servers().list().stream()
                    .filter(server -> server.getName().equals(resource.getName()))
                    .findFirst();
            Server server;
            if (existingServer.isPresent()) {
                ...
                ServerCreateBuilder serverCreateBuilder = Builders.server()
                        ...
                        .userData(new String(Base64.encodeBase64(cloudStack.getUserDataByType(group.getType()).getBytes())));
                ...
                for (CloudResource computeResource : context.getComputeResources(privateId)) {
                    if (computeResource.getType() == ResourceType.OPENSTACK_ATTACHED_DISK) {
                        VolumeSetAttributes volumeSetAttributes = computeResource.getParameter(CloudResource.ATTRIBUTES, VolumeSetAttributes.class);
                ...
                server = osClient.compute().servers().boot(serverCreate);
            }
            List<CloudResource> resources = new ArrayList<>();
            for (CloudResource computeResource : context.getComputeResources(privateId)) {
                computeResource.setInstanceId(server.getId());
                resources.add(computeResource);
            }
            resources.add(createPersistedResource(privateId, resource, group.getName(), server.getId(), server.getId(),
    ...
    public CloudResource delete(OpenStackContext context, AuthenticatedContext auth, CloudResource resource) {
        try {
            OSClient<?> osClient = createOSClient(auth);
            LOGGER.info("About to delete instance: [{}]", resource);
