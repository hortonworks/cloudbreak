...
public class OpenStackAuthenticator implements Authenticator {
    ...
    public AuthenticatedContext authenticate(CloudContext cloudContext, CloudCredential cloudCredential) {
        ...
        ensureClusterProxyRegistration(credential, cloudContext.getAccountId());
        return openStackClient.createAuthenticatedContext(cloudContext, cloudCredential);
    }

    private void ensureClusterProxyRegistration(KeystoneCredentialView credential, String accountId) {
        if (StringUtils.isNotBlank(credential.getRemoteEnvironmentCrn())
                && !clusterProxyService.isRegistered(accountId, credential.getName())) {
            clusterProxyService.registerServices(credential, accountId);
        }
    }
