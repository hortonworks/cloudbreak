...
public class OpenStackCredentialConnector implements CredentialConnector {
    ...
    public CloudCredentialStatus delete(@Nonnull AuthenticatedContext authenticatedContext) {
        ...
        if (StringUtils.isNotBlank(credential.getJumpgateEnvironmentCrn())) {
            LOGGER.info("Credential has jumpgateEnvironmentCrn set, deregistering services from cluster proxy");
            try {
                clusterProxyService.deregisterServices(authenticatedContext.getCloudCredential().getId());
