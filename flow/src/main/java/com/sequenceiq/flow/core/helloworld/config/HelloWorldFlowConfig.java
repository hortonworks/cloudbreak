...
import static com.sequenceiq.flow.core.helloworld.config.HelloWorldEvent.HELLOWORLD_FINISHED_EVENT;
...
import static com.sequenceiq.flow.core.helloworld.config.HelloWorldState.HELLO_WORLD_START_STATE;
...
public class HelloWorldFlowConfig extends AbstractFlowConfiguration<HelloWorldState, HelloWorldEvent> implements RetryableFlowConfiguration<HelloWorldEvent> {
    private static final List<Transition<HelloWorldState, HelloWorldEvent>> TRANSITIONS = new Transition.Builder<HelloWorldState, HelloWorldEvent>()
            ...
            .from(INIT_STATE).to(HELLO_WORLD_START_STATE).event(HELLOWORLD_TRIGGER_EVENT).noFailureEvent()
            .from(HELLO_WORLD_START_STATE).to(HELLO_WORLD_FINISHED_STATE).event(HELLOWORLD_FINISHED_EVENT).failureEvent(HELLOWORLD_SOMETHING_WENT_WRONG)
            .from(HELLO_WORLD_FINISHED_STATE).to(FINAL_STATE).event(FINALIZE_HELLOWORLD_EVENT).defaultFailureEvent()
            .build();
