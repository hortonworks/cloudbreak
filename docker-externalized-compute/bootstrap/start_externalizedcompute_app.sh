...
: ${SERVICE_SPECIFIC_CERT_DIR:=/externalizedcompute/certs}
...
mkdir -p /tmp/externalized-compute-log
...
if [ "$CRYPTOSENSE_ENABLED" == "true" ]; then
  EXTERNALIZED_COMPUTE_JAVA_OPTS="$EXTERNALIZED_COMPUTE_JAVA_OPTS -Dcryptosense.agent.out=/cryptosense/logs -javaagent:/cryptosense/cs-java-tracer.jar"
fi
...
EXTERNALIZED_COMPUTE_JAVA_OPTS="${EXTERNALIZED_COMPUTE_JAVA_OPTS} ${SECURITY_OPTS} ${JACOCO_AGENT_OPTIONS}"
