...
object TemplateRequests {
    ...
    val queryTemplates = http("query templates")
        ...
        .check(status.is(200),
                jsonPath("""$[?(@.cloudPlatform=="AZURE")].id""").optional.saveAs("azureTemplateId"),
                jsonPath("""$[?(@.cloudPlatform=="GCP")].id""").optional.saveAs("gcpTemplateId"),
                jsonPath("""$[?(@.cloudPlatform=="AWS")].id""").optional.saveAs("awsTemplateId"))
