        ...
        switch (roleType) {
            case SparkRoles.GATEWAY:
                ...
                if (storageLocationView.isPresent()) {
                    confClientConfigSafetyValveValue.append(SPARK3_YARN_ACCESS_DIR_PARAM +
                ...
                if (!confClientConfigSafetyValveValue.toString().isEmpty()) {
                    return List.of(config(SPARK3_CONF_CLIENT_SAFETY_VALVE, confClientConfigSafetyValveValue.toString()));
                }
                return List.of();
            default:
                return List.of();
