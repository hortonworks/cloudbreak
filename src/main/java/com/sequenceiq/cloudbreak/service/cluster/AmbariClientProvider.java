...
public class AmbariClientProvider {
    ...
    private static final String ADMIN_PRINCIPAL = "admin/admin";
    private static final String ADMIN_PRINCIPAL_PASSWORD = "admin";
     ...
     * @return client
     */
    public AmbariClient getAmbariClient(Stack stack) {
        ...
        if (stack.getCluster().isSecure()) {
            ambariClient.setKerberosSession(ADMIN_PRINCIPAL, ADMIN_PRINCIPAL_PASSWORD);
        }
        return ambariClient;
    }
    ...
    public AmbariClient getSecureAmbariClient(Stack stack) {
        AmbariClient ambariClient = getAmbariClient(stack.getAmbariIp(), stack.getUserName(), stack.getPassword());
        if (stack.getCluster().isSecure()) {
            setKerberosSession(ambariClient);
        }
        return ambariClient;
    }
     ...
     */
    public void setKerberosSession(AmbariClient client) {
        client.setKerberosSession(ADMIN_PRINCIPAL, ADMIN_PRINCIPAL_PASSWORD);
    }
}
