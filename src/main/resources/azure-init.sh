...
DOCKER_SUBNET_OF_FIRST_OTHER=$(echo $METADATA_RESULT | jq "$OTHER_INSTANCES_SELECTOR" | jq '.[0].privateIp' | sed s/\"//g)
SERF_JOIN_IP=${DOCKER_SUBNET_OF_FIRST_OTHER}
