...
start_ambari_agent() {
  ...
  sleep 2
  docker exec ambari-agent sed -i 's/ or not out.startswith(params.hdp_stack_version)//' /usr/lib/python2.6/site-packages/resource_management/libraries/functions/dynamic_variable_interpretation.py
}
