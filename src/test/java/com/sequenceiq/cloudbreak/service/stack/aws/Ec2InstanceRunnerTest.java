...
public class Ec2InstanceRunnerTest {
    ...
    public void setUp() {
        ...
        stackEvent = new Event<>(AwsStackTestUtil.createStack(user, credential, awsTemplate));
        output1 = new Output();
        output2 = new Output();
    ...
    @Test
    public void testAcceptEventStackWhenInstanceNotReachable() {
        //GIVEN
        describeInstanceStatusResult.setInstanceStatuses(new ArrayList<InstanceStatus>());
        given(awsStackUtil.createCloudFormationClient(Regions.DEFAULT_REGION, credential)).willReturn(cfClient);
        given(awsStackUtil.createEC2Client(Regions.DEFAULT_REGION, credential)).willReturn(ec2Client);
        given(cfClient.describeStacks(any(DescribeStacksRequest.class))).willReturn(stackResult);
        given(ec2Client.runInstances(any(RunInstancesRequest.class))).willReturn(runInstancesResult);
    ...
    @Test
    public void testAcceptEventWhenNotAbleToConnectCFClient() {
        //GIVEN
        describeInstanceStatusResult.setInstanceStatuses(new ArrayList<InstanceStatus>());
        given(awsStackUtil.createCloudFormationClient(Regions.DEFAULT_REGION, credential)).willThrow(new IllegalArgumentException());
        //WHEN.
        underTest.accept(stackEvent);
        //THEN
        verify(reactor, times(1)).notify(any(ReactorConfig.class), any(Event.class));
    ...
    private Reservation generateInstancesForReservation() {
        ...
        instances.add(new Instance().withInstanceId(generateRandomNumberStr()));
        instances.add(new Instance().withInstanceId(generateRandomNumberStr()));
        instances.add(new Instance().withInstanceId(generateRandomNumberStr()));
    ...
    private Collection<Reservation> generateDescribeInstancesForReservation() {
        ...
        for (int i = 0; i < MAX_ITERATION; i++) {
            ...
            List<Instance> instances = Lists.newArrayList();
            Instance instance = new Instance().withInstanceId(generateRandomNumberStr());
            ...
            instance.setPublicIpAddress(AwsStackTestUtil.AMBARI_IP);
    ...
    private Collection<InstanceStatus> generateInstanceStatuses() {
        ...
        for (int i = 0; i < MAX_ITERATION; i++) {
            ...
            is.setInstanceId(generateRandomNumberStr());
    ...
    private String generateRandomNumberStr() {
