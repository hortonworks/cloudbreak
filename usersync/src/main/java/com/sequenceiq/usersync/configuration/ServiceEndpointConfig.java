...
import com.sequenceiq.usersync.service.registry.DNSServiceAddressResolver;
import com.sequenceiq.usersync.service.registry.RetryingServiceAddressResolver;
import com.sequenceiq.usersync.service.registry.ServiceAddressResolver;
import com.sequenceiq.usersync.service.registry.ServiceAddressResolvingException;

@Configuration
public class ServiceEndpointConfig {
    @Value("${usersync.address.resolving.timeout:60000}")
    private int resolvingTimeout;

    @Value("${usersync.db.port.5432.tcp.addr:}")
    private String dbHost;

    @Value("${usersync.db.port.5432.tcp.port:}")
    private String dbPort;

    @Value("${usersync.db.serviceid:}")
    private String databaseId;

    @Value("${usersync.cloudbreak.url:}")
    private String cloudbreakUrl;

    @Value("${usersync.cloudbreak.serviceid:}")
