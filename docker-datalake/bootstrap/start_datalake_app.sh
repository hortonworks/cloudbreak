...
: ${SERVICE_SPECIFIC_CERT_DIR:=/datalake/certs}
...
if [ "$CRYPTOSENSE_ENABLED" == "true" ]; then
  DATALAKE_JAVA_OPTS="$DATALAKE_JAVA_OPTS -Dcryptosense.agent.out=/cryptosense/logs -javaagent:/cryptosense/cs-java-tracer.jar"
fi
...
DATALAKE_JAVA_OPTS="${DATALAKE_JAVA_OPTS} ${SECURITY_OPTS} ${JACOCO_AGENT_OPTIONS}"
