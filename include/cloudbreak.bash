...
cloudbreak-conf-tags() {
    ...
    env-import DOCKER_TAG_PERISCOPE 1.14.0-dev.89
    env-import DOCKER_TAG_CLOUDBREAK 1.14.0-dev.89
    env-import DOCKER_TAG_ULUWATU 1.14.0-dev.89
    env-import DOCKER_TAG_SULTANS 1.14.0-dev.89
    env-import DOCKER_TAG_CLOUDBREAK_SHELL 1.14.0-dev.89
...
util-add-default-user() {
    ...
    local bearer=$(docker exec -it $container curl -s "$address/oauth/token?grant_type=client_credentials&token_format=opaque" -u "${UAA_SULTANS_ID}:${UAA_SULTANS_SECRET}" | jq -r .access_token 2>/dev/null)
    ...
    if [[ "$user_id" != "null" ]]; then
        local groups=$(docker exec -it $container  curl -s "$address/Groups" -H "Authorization: Bearer $bearer" | jq ".resources[] | {id, displayName}")
        for group in ${UAA_DEFAULT_USER_GROUPS//,/ }; do
            debug "Adding user to group $group"
            group_id=$(echo $groups | jq . | grep -B2 -A1 '"'$group'"' | jq -r .id)
            docker exec -it $container curl -s "$address/Groups/$group_id/members" -X POST -H "Authorization: Bearer $bearer" -H 'Content-Type: application/json' -d '{"origin":"uaa","type":"USER","value":"'$user_id'"}' &>/dev/null
        done
    else
