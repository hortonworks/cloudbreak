...
public class StructuredSynchronizerJobService implements JobSchedulerService {
    ...
    public <T> void schedule(JobResourceAdapter<T> resource, boolean withDelay) {
        if (structuredSynchronizerConfig.isStructuredSyncEnabled()) {
            ...
            try {
                ...
                LOGGER.info("Scheduling structured event sync job for environment with key: '{}' and group: '{}'", jobKey.getName(), jobKey.getGroup());
                scheduler.scheduleJob(jobDetail, trigger);
            } catch (Exception e) {
                LOGGER.error(String.format("Error during scheduling quartz job: %s", id), e);
            }
        } else {
            LOGGER.info("Structured sync is disabled, not scheduling job for environment with key: '{}'", resource.getJobResource().getLocalId());
        }
    }
    ...
    public void unschedule(String id) {
        try {
            ...
            LOGGER.info("Unscheduling structured event sync job for environment with key: '{}' and group: '{}'", jobKey.getName(), jobKey.getGroup());
    ...
    private <T> JobDetail buildJobDetail(JobResourceAdapter<T> resourceAdapter) {
        return JobBuilder.newJob(resourceAdapter.getJobClassForResource())
                ...
                .withDescription("Creating Environment Structured Synchronization Event")
    ...
    protected Trigger buildJobTrigger(JobDetail jobDetail) {
        return TriggerBuilder.newTrigger()
                ...
                .withDescription("Environment Structured Synchronization Event Trigger")
