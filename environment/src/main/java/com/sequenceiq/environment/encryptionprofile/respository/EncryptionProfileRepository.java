...
public interface EncryptionProfileRepository extends JpaRepository<EncryptionProfile, Long> {
    ...
    @Query("SELECT e.name as name " +
            "FROM Environment e " +
            "JOIN EncryptionProfile ep ON ep.resourceCrn =  e.encryptionProfileCrn " +
            "WHERE ep.resourceCrn = :crn " +
            "AND e.archived = FALSE")
    List<String> findAllEnvNamesByEncryptionProfileCrn(@Param("crn") String crn);
}
