...
abstract class AbstractLoadBalancerUpdateAction<P extends ResourceCrnPayload>
    extends AbstractAction<LoadBalancerUpdateState, LoadBalancerUpdateStateSelectors, CommonContext, P> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractLoadBalancerUpdateAction.class);

    AbstractLoadBalancerUpdateAction(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected CommonContext createFlowContext(FlowParameters flowParameters,
            StateContext<LoadBalancerUpdateState, LoadBalancerUpdateStateSelectors> stateContext, P payload) {
