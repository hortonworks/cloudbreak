...
abstract class AbstractEnvironmentVerticalScaleAction<P extends ResourceCrnPayload>
        extends AbstractAction<EnvironmentVerticalScaleState, EnvironmentVerticalScaleStateSelectors, CommonContext, P> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractEnvironmentVerticalScaleAction.class);

    protected AbstractEnvironmentVerticalScaleAction(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected CommonContext createFlowContext(FlowParameters flowParameters,
            StateContext<EnvironmentVerticalScaleState, EnvironmentVerticalScaleStateSelectors> stateContext,
            P payload) {
