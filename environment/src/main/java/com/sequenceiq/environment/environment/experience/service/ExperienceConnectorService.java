...
import com.sequenceiq.environment.environment.experience.response.Experience;
...
public class ExperienceConnectorService {
    ...
    public ExperienceConnectorService(@Value("${xp.path.componentToReplace}") String componentToReplaceInPath, Client client) {
        this.client = client;
    ...
    @NotNull Set<String> getWorkspaceNamesConnectedToEnv(String experienceBasePath, String environmentCrn) {
        ...
        if (result != null) {
            ...
            return response.map(ExperienceConnectorService::getExperienceNamesFromResponse).orElseGet(Set::of);
        }
    ...
    private static Set<String> getExperienceNamesFromResponse(ExperienceCallResponse experienceCallResponse) {
        return experienceCallResponse.getResults().stream().map(Experience::getName).collect(Collectors.toSet());
    }
