...
abstract class AbstractEnableEncryptionProfileActions<P extends ResourceCrnPayload>
        extends AbstractAction<EnabledEncryptionProfileState, EnableEncryptionProfileStateSelectors, CommonContext, P> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractEnableEncryptionProfileActions.class);

    protected AbstractEnableEncryptionProfileActions(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected CommonContext createFlowContext(FlowParameters flowParameters,
            StateContext<EnabledEncryptionProfileState, EnableEncryptionProfileStateSelectors> stateContext,
            P payload) {
