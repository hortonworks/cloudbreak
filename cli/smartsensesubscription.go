...
func getAvailableSmartSenseSubscription(c *cli.Context) (*models_cloudbreak.SmartSenseSubscriptionJSON, error) {
	cbClient := NewCloudbreakOAuth2HTTPClient(c.String(FlServer.Name), c.String(FlUsername.Name), c.String(FlPassword.Name))
	sss, err := cbClient.Cloudbreak.Smartsensesubscriptions.GetPrivateSmartSenseSubscriptions(&s.GetPrivateSmartSenseSubscriptionsParams{})
	if err != nil {
		return nil, err
	} else if len(sss.Payload) == 0 {
		return nil, errors.New("SmartSense subscription is missing")
	} else if *sss.Payload[0].AutoGenerated {
		return nil, errors.New("SmartSense subscription generated by Cloudbreak")
	}
	return sss.Payload[0], nil
}
