package cli

import (
	"strconv"
	"strings"
	"testing"

	"github.com/hortonworks/cb-cli/cli/types"
	"github.com/hortonworks/cb-cli/cli/utils"
	"github.com/hortonworks/cb-cli/client_cloudbreak/v1blueprints"
	"github.com/hortonworks/cb-cli/models_cloudbreak"
)

type mockBlueprintsClient struct {
}

func (*mockBlueprintsClient) GetPrivatesBlueprint(params *v1blueprints.GetPrivatesBlueprintParams) (*v1blueprints.GetPrivatesBlueprintOK, error) {
	resp := make([]*models_cloudbreak.BlueprintResponse, 0)
	for i := 0; i < 2; i++ {
		id := int64(i)
		resp = append(resp, &models_cloudbreak.BlueprintResponse{
			ID:              id,
			Name:            &(&types.S{S: "name" + strconv.Itoa(i)}).S,
			Description:     &(&types.S{S: "desc" + strconv.Itoa(i)}).S,
			HostGroupCount:  3,
			Status:          "USER_MANAGED",
			AmbariBlueprint: "",
		})
	}
	return &v1blueprints.GetPrivatesBlueprintOK{Payload: resp}, nil
}

func (*mockBlueprintsClient) PostPrivateBlueprint(params *v1blueprints.PostPrivateBlueprintParams) (*v1blueprints.PostPrivateBlueprintOK, error) {
	return &v1blueprints.PostPrivateBlueprintOK{Payload: &models_cloudbreak.BlueprintResponse{ID: int64(1)}}, nil
}

func (*mockBlueprintsClient) PostPublicBlueprint(params *v1blueprints.PostPublicBlueprintParams) (*v1blueprints.PostPublicBlueprintOK, error) {
	return &v1blueprints.PostPublicBlueprintOK{Payload: &models_cloudbreak.BlueprintResponse{ID: int64(1)}}, nil
}

func (*mockBlueprintsClient) DeletePrivateBlueprint(params *v1blueprints.DeletePrivateBlueprintParams) error {
	return nil
}

func TestListBlueprintsImpl(t *testing.T) {
	t.Parallel()

	var rows []utils.Row

	listBlueprintsImpl(new(mockBlueprintsClient), func(h []string, r []utils.Row) { rows = r })

	if len(rows) != 2 {
		t.Fatalf("row number doesn't match 2 == %d", len(rows))
	}

	for i, r := range rows {
		expected := []string{"name" + strconv.Itoa(i), "desc" + strconv.Itoa(i), "2.6", "3", "USER_MANAGED"}
		if strings.Join(r.DataAsStringArray(), "") != strings.Join(expected, "") {
			t.Errorf("row data not match %s == %s", expected, r.DataAsStringArray())
		}
	}
}
