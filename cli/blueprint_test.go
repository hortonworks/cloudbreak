package cli

import (
	"strconv"
	"strings"
	"testing"

	"github.com/hortonworks/cb-cli/cli/types"
	"github.com/hortonworks/cb-cli/cli/utils"
	"github.com/hortonworks/cb-cli/client_cloudbreak/v3_organization_id_blueprints"
	"github.com/hortonworks/cb-cli/models_cloudbreak"
)

type mockBlueprintsClient struct {
}

func (*mockBlueprintsClient) ListBlueprintsByOrganization(params *v3_organization_id_blueprints.ListBlueprintsByOrganizationParams) (*v3_organization_id_blueprints.ListBlueprintsByOrganizationOK, error) {
	resp := make([]*models_cloudbreak.BlueprintResponse, 0)
	for i := 0; i < 2; i++ {
		id := int64(i)
		resp = append(resp, &models_cloudbreak.BlueprintResponse{
			ID:              id,
			Name:            &(&types.S{S: "name" + strconv.Itoa(i)}).S,
			Description:     &(&types.S{S: "desc" + strconv.Itoa(i)}).S,
			HostGroupCount:  3,
			Status:          "USER_MANAGED",
			AmbariBlueprint: "",
		})
	}
	return &v3_organization_id_blueprints.ListBlueprintsByOrganizationOK{Payload: resp}, nil
}

func (*mockBlueprintsClient) CreateBlueprintInOrganization(params *v3_organization_id_blueprints.CreateBlueprintInOrganizationParams) (*v3_organization_id_blueprints.CreateBlueprintInOrganizationOK, error) {
	return &v3_organization_id_blueprints.CreateBlueprintInOrganizationOK{Payload: &models_cloudbreak.BlueprintResponse{ID: int64(1)}}, nil
}

func (*mockBlueprintsClient) DeleteBlueprintInOrganization(params *v3_organization_id_blueprints.DeleteBlueprintInOrganizationParams) (*v3_organization_id_blueprints.DeleteBlueprintInOrganizationOK, error) {
	return nil, nil
}

func TestListBlueprintsImpl(t *testing.T) {
	t.Parallel()

	var rows []utils.Row

	orgId := int64(2)
	listBlueprintsImpl(orgId, new(mockBlueprintsClient), func(h []string, r []utils.Row) { rows = r })

	if len(rows) != 2 {
		t.Fatalf("row number doesn't match 2 == %d", len(rows))
	}

	for i, r := range rows {
		expected := []string{"name" + strconv.Itoa(i), "desc" + strconv.Itoa(i), "2.6", "3", "USER_MANAGED"}
		if strings.Join(r.DataAsStringArray(), "") != strings.Join(expected, "") {
			t.Errorf("row data not match %s == %s", expected, r.DataAsStringArray())
		}
	}
}
