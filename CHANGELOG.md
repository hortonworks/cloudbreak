...
- Update Ambari and HDP 2.6 default versions
- Move to centos 7.3 base to be able to encrypt os disk
- Disable ssh key verification on Openstack
- Support more than one hostgroup for one Azure Availability Set
- Handling expressions that's generated by Ambari in exported blueprints
- Kdc-type default value is mit-kdc insteadof null
- Include ADLS tracking tags in Azure
- Move HSI configs to hive-interactive-site
- Remove grafana (not used for anything atm)

### Added

- Support Azure multiple subnet
- Azure premium managed disk support
- Added azure storage account encryption
- Add new default configs for hive and tez

### Fixed

- Fix public in account, disappeared from Templates and SecurityGroups
- Fix credential creation ui, scrambled if select yarn after azure selected
- Fix cannot downscale in Azure if using managed disks
- Fix cannot create cluster with GCS fs, hadoop-classpath.sh does not run properly, GCS connector missing from HADOOP_CLASSPATH
- Fix upscale issue
- Fix "Availability set cannot be found" error
