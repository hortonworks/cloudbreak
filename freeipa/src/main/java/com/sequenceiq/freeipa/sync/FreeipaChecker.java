...
public class FreeipaChecker {
    ...
    public SyncResult getStatus(Stack stack, Set<InstanceMetaData> checkableInstances) {
        try {
            ...
            String postFix = "";
            int expectedRunningInstances = stack.getInstanceGroups().stream()
                    .map(InstanceGroup::getNodeCount)
                    .collect(Collectors.summingInt(Integer::intValue));
            Boolean result = responses.stream().allMatch(RPCResponse::getResult);
            if (result && responses.size() == expectedRunningInstances) {
