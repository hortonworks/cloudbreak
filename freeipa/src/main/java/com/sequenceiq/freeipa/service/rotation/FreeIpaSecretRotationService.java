...
public class FreeIpaSecretRotationService implements SecretRotationFlowEventProvider {
    ...
    public StepProgressResponse getProgress(String crn, String secret) {
        SecretType secretType = SecretTypeConverter.mapSecretType(secret, enabledSecretTypes.stream().map(SecretType::getClass).collect(Collectors.toSet()));
    ...
    public Selectable getSaltUpdateTriggerEvent(SecretRotationFlowChainTriggerEvent event) {
        return new SaltUpdateTriggerEvent(event.getResourceId(), event.accepted(), true, false, null, skipSaltHighstate(event));
    }
}
