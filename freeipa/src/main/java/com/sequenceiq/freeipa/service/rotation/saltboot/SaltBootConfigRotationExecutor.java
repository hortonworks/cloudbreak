...
public class SaltBootConfigRotationExecutor extends AbstractRotationExecutor<SaltBootConfigRotationContext> {
    ...
    private void restartSaltBootService(SaltBootUpdateConfiguration SaltBootConfig, GatewayConfig gatewayConfig) {
        if (CollectionUtils.isNotEmpty(SaltBootConfig.serviceRestartActions())) {
            try {
                LOGGER.info("Executing restart actions {} on hosts {}", SaltBootConfig.serviceRestartActions(), SaltBootConfig.targetFqdns());
                hostOrchestrator.executeSaltState(
                        gatewayConfig,
                        SaltBootConfig.targetFqdns(),
                        SaltBootConfig.serviceRestartActions(),
                        SaltBootConfig.exitCriteriaModel(),
                        Optional.of(SaltBootConfig.maxRetryCount()),
                        Optional.of(SaltBootConfig.maxRetryCount()));
            } catch (CloudbreakOrchestratorFailedException e) {
                throw new SecretRotationException(e, getType());
            }
