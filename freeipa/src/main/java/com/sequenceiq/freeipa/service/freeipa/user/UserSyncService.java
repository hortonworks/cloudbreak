...
public class UserSyncService {
    ...
    public Operation synchronizeUsers(String accountId, String actorCrn, Set<String> environmentCrnFilter,
        ...
        UserSyncOptions userSyncOptions = getUserSyncOptions(accountId, userSyncFilter.isFullSync(), forceWorkloadCredentialsUpdate);
        return performSyncForStacks(accountId, actorCrn, userSyncFilter, userSyncOptions, stacks);
    }
    ...
    private Operation performSyncForStacks(String accountId, String actorCrn, UserSyncRequestFilter userSyncFilter, UserSyncOptions userSyncOptions,
            List<Stack> stacks) {
        ...
        if (operationState == OperationState.RUNNING) {
            tryWithOperationCleanup(operationId, accountId, () ->
                    ThreadBasedUserCrnProvider.doAs(INTERNAL_ACTOR_CRN, () -> {
                        ...
                        asyncSynchronizeUsers(operation.getOperationId(), accountId, actorCrn, stacks, userSyncFilter, userSyncOptions);
                    }));
        }
    ...
    void asyncSynchronizeUsers(String operationId, String accountId, String actorCrn, List<Stack> stacks, UserSyncRequestFilter userSyncFilter,
        ...
        try {
            ...
            asyncTaskExecutor.submit(() -> internalSynchronizeUsers(operationId, accountId, actorCrn, stacks, userSyncFilter, userSyncOptions));
        } finally {
    ...
    private void internalSynchronizeUsers(String operationId, String accountId, String actorCrn, List<Stack> stacks, UserSyncRequestFilter userSyncFilter,
        ...
        tryWithOperationCleanup(operationId, accountId, () -> {
            ...
            UmsEventGenerationIds umsEventGenerationIds = userSyncOptions.fullSync() ?
                    umsEventGenerationIdsProvider.getEventGenerationIds(accountId, requestId) :
                    null;

            LogEvent logUserSyncEvent = userSyncOptions.fullSync() ? LogEvent.FULL_USER_SYNC : LogEvent.PARTIAL_USER_SYNC;
            ...
            if (userSyncFilter.getDeletedWorkloadUser().isEmpty()) {
                LogEvent logRetrieveUmsEvent = userSyncOptions.fullSync() ? LogEvent.RETRIEVE_FULL_UMS_STATE : LogEvent.RETRIEVE_PARTIAL_UMS_STATE;
                ...
                statusFutures = stacks.stream()
                        .collect(Collectors.toMap(Stack::getEnvironmentCrn,
                                stack -> asyncSynchronizeStack(stack, envToUmsStateMap.get(stack.getEnvironmentCrn()), umsEventGenerationIds,
                                        userSyncOptions, operationId, accountId)));
            } else {
    ...
    private Future<SyncStatusDetail> asyncSynchronizeStack(Stack stack, UmsUsersState umsUsersState, UmsEventGenerationIds umsEventGenerationIds,
            UserSyncOptions userSyncOptions, String operationId, String accountId) {
        return asyncTaskExecutor.submit(() -> {
            SyncStatusDetail statusDetail = internalSynchronizeStack(stack, umsUsersState, userSyncOptions);
            if (userSyncOptions.fullSync() && statusDetail.getStatus() == SynchronizationStatus.COMPLETED) {
                UserSyncStatus userSyncStatus = userSyncStatusService.getOrCreateForStack(stack);
    ...
    private SyncStatusDetail internalSynchronizeStack(Stack stack, UmsUsersState umsUsersState, UserSyncOptions userSyncOptions) {
        MDCBuilder.buildMdcContext(stack);
        String environmentCrn = stack.getEnvironmentCrn();
        Multimap<String, String> warnings = ArrayListMultimap.create();
        try {
            FreeIpaClient freeIpaClient = freeIpaClientFactory.getFreeIpaClientForStack(stack);
            UsersStateDifference usersStateDifferenceBeforeSync = compareUmsAndFreeIpa(umsUsersState, userSyncOptions, freeIpaClient);
            applyDifference(umsUsersState, environmentCrn, warnings, usersStateDifferenceBeforeSync, userSyncOptions, freeIpaClient);

            retrySyncIfBatchCallHasWarnings(stack, umsUsersState, warnings, userSyncOptions, freeIpaClient, usersStateDifferenceBeforeSync);
            ...
            if (userSyncOptions.fullSync() && entitlementService.cloudIdentityMappingEnabled(stack.getAccountId())) {
    ...
    private void retrySyncIfBatchCallHasWarnings(Stack stack, UmsUsersState umsUsersState, Multimap<String, String> warnings,
        ...
        if (options.fullSync() && !warnings.isEmpty() && options.fmsToFreeIpaBatchCallEnabled()) {
    ...
    private UsersStateDifference compareUmsAndFreeIpa(UmsUsersState umsUsersState, UserSyncOptions options,
        ...
        LogEvent logEvent = options.fullSync() ? LogEvent.RETRIEVE_FULL_IPA_STATE : LogEvent.RETRIEVE_PARTIAL_IPA_STATE;
        LOGGER.debug("Starting {} ...", logEvent);
        UsersState ipaUsersState = getIpaUserState(freeIpaClient, umsUsersState, options.fullSync());
    ...
    private void applyDifference(UmsUsersState umsUsersState, String environmentCrn, Multimap<String, String> warnings,
        ...
        applyStateDifferenceToIpa(environmentCrn, freeIpaClient, usersStateDifference, warnings::put, options.fmsToFreeIpaBatchCallEnabled());
