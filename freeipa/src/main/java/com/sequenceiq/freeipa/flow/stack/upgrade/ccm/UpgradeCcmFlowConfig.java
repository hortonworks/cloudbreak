...
import static com.sequenceiq.freeipa.flow.stack.upgrade.ccm.UpgradeCcmState.UPGRADE_CCM_HEALTH_CHECK_STATE;
import static com.sequenceiq.freeipa.flow.stack.upgrade.ccm.UpgradeCcmState.UPGRADE_CCM_PUSH_SALT_STATES_STATE;
import static com.sequenceiq.freeipa.flow.stack.upgrade.ccm.UpgradeCcmState.UPGRADE_CCM_RECONFIGURE_STATE;
import static com.sequenceiq.freeipa.flow.stack.upgrade.ccm.UpgradeCcmState.UPGRADE_CCM_REGISTER_CCM_STATE;
...
public class UpgradeCcmFlowConfig extends AbstractFlowConfiguration<UpgradeCcmState, UpgradeCcmStateSelector>
    ...
    private static final List<Transition<UpgradeCcmState, UpgradeCcmStateSelector>> TRANSITIONS =
            new Transition.Builder<UpgradeCcmState, UpgradeCcmStateSelector>().defaultFailureEvent(UPGRADE_CCM_FAILED_EVENT)
                    ...
                    .from(UPGRADE_CCM_CHECK_PREREQUISITES_STATE).to(UPGRADE_CCM_CHANGE_TUNNEL_STATE).event(UPGRADE_CCM_CHECK_PREREQUISITES_FINISHED_EVENT).
                        defaultFailureEvent()
                    .from(UPGRADE_CCM_CHANGE_TUNNEL_STATE).to(UPGRADE_CCM_PUSH_SALT_STATES_STATE).event(UPGRADE_CCM_TUNNEL_CHANGE_FINISHED_EVENT)
                        .defaultFailureEvent()
                    .from(UPGRADE_CCM_PUSH_SALT_STATES_STATE).to(UPGRADE_CCM_UPGRADE_STATE).event(UPGRADE_CCM_PUSH_SALT_STATES_FINISHED_EVENT)
                        .defaultFailureEvent()
                    .from(UPGRADE_CCM_UPGRADE_STATE).to(UPGRADE_CCM_RECONFIGURE_STATE).event(UPGRADE_CCM_UPGRADE_FINISHED_EVENT).defaultFailureEvent()
                    .from(UPGRADE_CCM_RECONFIGURE_STATE).to(UPGRADE_CCM_REGISTER_CCM_STATE).event(UPGRADE_CCM_RECONFIGURE_FINISHED_EVENT).defaultFailureEvent()
                    .from(UPGRADE_CCM_REGISTER_CCM_STATE).to(UPGRADE_CCM_HEALTH_CHECK_STATE).event(UPGRADE_CCM_REGISTER_CCM_FINISHED_EVENT).defaultFailureEvent()
                    .from(UPGRADE_CCM_HEALTH_CHECK_STATE).to(UPGRADE_CCM_REMOVE_MINA_STATE).event(UPGRADE_CCM_HEALTH_CHECK_FINISHED_EVENT).defaultFailureEvent()
                    .from(UPGRADE_CCM_REMOVE_MINA_STATE).to(UPGRADE_CCM_FINISHED_STATE).event(UPGRADE_CCM_REMOVE_MINA_FINISHED_EVENT).defaultFailureEvent()
