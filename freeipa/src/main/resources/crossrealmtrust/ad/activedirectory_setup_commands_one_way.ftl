...
:: Set up one-way trust from the Active Directory server towards the FreeIPA
...
netdom query /Domain:%AD_DOMAIN% /Direct TRUST | findstr %IPA_DOMAIN% > NUL 2>&1 && (
  ...
  netdom trust %IPA_DOMAIN% /Domain:%AD_DOMAIN% /Add /ForestTRANsitive:yes /PasswordT:%TRUST_SECRET% /oneside:trusted /verbose || (
