...
class ModifyProxyOperationAcceptorTest {
    ...
    @InjectMocks
    private ModifyProxyOperationAcceptor underTest;

    @Test
    void selectorTest() {
        assertThat(underTest.selector()).isEqualTo(OperationType.MODIFY_PROXY_CONFIG);
    }
    ...
    void acceptTestWhenValidationFailure(String testCaseName, List<String> environmentList) {
        ...
        assertThat(rejectionMessage.get()).isEqualTo("Proxy config modification must be invoked for a single environment!");
    }

    @Test
    void acceptTestWhenHasRunningOperationForSameStack() {
        Operation operation = createOperation(ID_1, List.of(ENVIRONMENT_CRN_1));
        when(operationRepository.findRunningByAccountIdAndType(ACCOUNT_ID, OperationType.MODIFY_PROXY_CONFIG))
        ...
        assertThat(rejectionMessage).isNotNull();
        assertThat(rejectionMessage).isPresent();
        assertThat(rejectionMessage.get()).isEqualTo("There is already a running proxy config modification for FreeIPA stack");
    }
    ...
    void acceptTestWhenSuccess(String testCaseName, List<Operation> runningOperations) {
        Operation operation = createOperation(ID_1, List.of(ENVIRONMENT_CRN_1));
        when(operationRepository.findRunningByAccountIdAndType(ACCOUNT_ID, OperationType.MODIFY_PROXY_CONFIG)).thenReturn(runningOperations);
