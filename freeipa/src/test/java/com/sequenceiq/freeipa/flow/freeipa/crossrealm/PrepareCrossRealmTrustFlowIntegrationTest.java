...
import static com.sequenceiq.freeipa.api.v1.freeipa.stack.model.common.DetailedStackStatus.PREPARE_CROSS_REALM_TRUST_FAILED;
import static com.sequenceiq.freeipa.api.v1.freeipa.stack.model.common.DetailedStackStatus.PREPARE_CROSS_REALM_TRUST_IN_PROGRESS;
...
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.FreeIpaPrepareCrossRealmTrustFlowConfig;
...
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.converter.CrossRealmTrustValidationFailedToPrepareCrossRealmTrustFailureEvent;
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.event.PrepareCrossRealmTrustEvent;
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.handler.ConfigureDnsHandler;
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.handler.PrepareIpaServerHandler;
import com.sequenceiq.freeipa.flow.freeipa.crossrealm.trust.handler.ValidationHandler;
...
class PrepareCrossRealmTrustFlowIntegrationTest {
    ...
    void testPrepareCrossRealmTrustWhenSuccessful() {
        ...
        stackStatusVerify.verify(stackUpdater).updateStackStatus(stack, PREPARE_CROSS_REALM_TRUST_IN_PROGRESS, "Cross-realm trust validation");
        stackStatusVerify.verify(stackUpdater).updateStackStatus(stack, PREPARE_CROSS_REALM_TRUST_IN_PROGRESS, "Prepare IPA server");
        stackStatusVerify.verify(stackUpdater).updateStackStatus(stack, PREPARE_CROSS_REALM_TRUST_IN_PROGRESS, "Configuring DNS");
        stackStatusVerify.verify(stackUpdater)
                .updateStackStatus(stack, DetailedStackStatus.PREPARE_CROSS_REALM_TRUST_PENDING, "Prepare cross-realm trust finished");
    }

    @Test
    public void testValidationFails() {
        doThrow(new CloudbreakServiceException("Cross-realm validation failed")).when(crossRealmValidationService).validateCrossRealmPreparation(STACK_ID);
        testFlow();
        InOrder stackStatusVerify = inOrder(stackUpdater);

        stackStatusVerify.verify(stackUpdater).updateStackStatus(stack, PREPARE_CROSS_REALM_TRUST_IN_PROGRESS, "Cross-realm trust validation");
        stackStatusVerify.verify(stackUpdater).updateStackStatus(stack, PREPARE_CROSS_REALM_TRUST_FAILED,
                "Failed to prepare cross-realm trust FreeIPA: Cross-realm validation failed");
    }

    @Test
    public void testPrepareIpaServerFails() {
    ...
    @Test
    public void testConfigureDnsServerFails() {
    ...
    private FlowIdentifier triggerFlow() {
        PrepareCrossRealmTrustEvent prepareCrossRealmTrustEvent = new PrepareCrossRealmTrustEvent(STACK_ID, OPERATION_ID);
    ...
    @TestConfiguration
    @Import(value = {
            FreeIpaPrepareCrossRealmTrustFlowConfig.class,
            FlowIntegrationTestConfig.class,
            WebApplicationExceptionMessageExtractor.class,
            PrepareCrossRealmTrustValidationAction.class,
            PrepareCrossRealmTrustFailedAction.class,
            PrepareCrossRealmTrustPrepareIpaServerAction.class,
            PrepareCrossRealmTrustConfigureDnsAction.class,
            PrepareCrossRealmTrustFinishedAction.class,
            PrepareCrossRealmTrustFailedAction.class,
            CrossRealmTrustValidationFailedToPrepareCrossRealmTrustFailureEvent.class,
