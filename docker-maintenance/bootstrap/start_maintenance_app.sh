...
: ${SERVICE_SPECIFIC_CERT_DIR:=/maintenance/certs}
...
java /ImportCerts.java "$CACERTS_WRITABLE" changeit "$TRUSTED_CERT_DIR" "$SERVICE_SPECIFIC_CERT_DIR"
...
echo "Starting the Maintenance application..."

MAINTENANCE_JAVA_OPTS="$MAINTENANCE_JAVA_OPTS -XX:+ExitOnOutOfMemoryError"

set -x
if [ "$SECURE_RANDOM" == "false" ]; then
  MAINTENANCE_JAVA_OPTS="$MAINTENANCE_JAVA_OPTS -Djava.security.egd=file:/dev/./urandom"
fi

if [ "$CRYPTOSENSE_ENABLED" == "true" ]; then
  MAINTENANCE_JAVA_OPTS="$MAINTENANCE_JAVA_OPTS -Dcryptosense.agent.out=/cryptosense/logs -javaagent:/cryptosense/cs-java-tracer.jar"
fi
...
MAINTENANCE_JAVA_OPTS="${MAINTENANCE_JAVA_OPTS} ${SECURITY_OPTS} ${JACOCO_AGENT_OPTIONS}"

eval "(java $MAINTENANCE_JAVA_OPTS -jar /maintenance.jar) & JAVAPID=\$!; trap \"kill \$JAVAPID; wait \$JAVAPID\" SIGINT SIGTERM; wait \$JAVAPID"
