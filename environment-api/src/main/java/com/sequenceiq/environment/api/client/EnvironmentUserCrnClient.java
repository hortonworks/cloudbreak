...
class EnvironmentUserCrnClient implements EnvironmentClient {
    ...
    EnvironmentUserCrnClient(String environmentAddress, ConfigKey configKey) {
        Client client = RestClientUtil.get(configKey);
        webTarget = client.target(environmentAddress);
        crn = configKey.getToken();
        LOGGER.info("EnvironmentUserCrnClient has been created with token. environment service: {}, configKey: {}", environmentAddress,
                configKey);
    }
    ...
    public LdapConfigV1Endpoint ldapConfigV1Endpoint() {
