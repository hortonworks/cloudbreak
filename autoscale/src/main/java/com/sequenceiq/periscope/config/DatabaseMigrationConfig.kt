...
class DatabaseMigrationConfig {
    ...
    fun databaseUpMigration(): UpOperation {
        ...
        if (schemaMigrationEnabled) {
            ...
            val upMigrationsLoader = upMigrationLoader()
            upOperation = upOperation.operate(dataSourceConnectionProvider, upMigrationsLoader, operationOption, PrintStream(upOutStream))
            val pendingOperationsLoader = pendingMigrationLoader()
            pendingOperation.operate(dataSourceConnectionProvider, pendingOperationsLoader, operationOption, PrintStream(pendingOutStream))
    ...
    companion object {
        ...
        private val DEFAULT_SCHEMA_LOCATION_IN_CONTAINER = "/schema"
        private val SCHEMA_IN_CONTAINER = "container"
        private val DEFAULT_SCHEMA_LOCATION_IN_SOURCE = "autoscale/src/main/resources/schema"
