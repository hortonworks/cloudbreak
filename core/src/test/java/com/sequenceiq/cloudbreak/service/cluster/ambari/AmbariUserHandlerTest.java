...
public class AmbariUserHandlerTest {
    ...
    @Inject
    private AmbariUserHandler underTest;
    ...
    public void testCreateAmbariUserWhenNoExceptionOccuredThenEverythingShouldWorksFine() throws CloudbreakException {
        ...
        when(ambariClientFactory.getAmbariClient(eq(stack), eq(newUserName), eq(newPassword))).thenReturn(ambariClient);
    ...
    public void testChangeAmbariPasswordWhenNoExceptionOccuredThenEverythingShouldWorksFine() throws CloudbreakException {
        ...
        when(ambariClientFactory.getAmbariClient(eq(stack), eq(userName), eq(newPassword))).thenReturn(ambariClient);

        AmbariClient ambariClientResult = underTest.changeAmbariPassword(userName, oldPassword, newPassword, stack, ambariClient);

        Assert.assertEquals(ambariClient, ambariClientResult);

        verify(ambariClient, times(1)).changePassword(userName, oldPassword, newPassword, true);
        verify(ambariClientFactory, times(1)).getAmbariClient(stack, userName, newPassword);
