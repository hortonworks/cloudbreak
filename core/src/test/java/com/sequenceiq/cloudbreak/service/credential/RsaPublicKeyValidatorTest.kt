...
class RsaPublicKeyValidatorTest {

    private var rsaPublicKeyValidator: RsaPublicKeyValidator? = null

    @Before
    fun setUp() {
        rsaPublicKeyValidator = RsaPublicKeyValidator()
    }

    @Test
    fun validPublicKeyWillNotFail() {
    ...
    fun inValidPublicKeyWillCorrectWhenMissingUser() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
                        "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQCasJyap4swb4Hk4xOlnF3OmKVwzmv2e053yrtvcUPaxCeboSltOBReuT"
                                + "QxX+kYCgKCdtEwpIvEDXk16T6nCI4tSptAalFgpUWn+JOysCuLuWnwrk6mSKOzEiPYCrB54444mDY6rbBDSRuE/V"
    ...
    fun inValidPublicKeyWillFailWhenMissingSshRsa() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
    ...
    @Test(expected = BadRequestException::class)
    fun inValidPublicKeyWillFailWhenTooShortBody() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
    ...
    @Test(expected = BadRequestException::class)
    fun inValidPublicKeyWillFailWhenMissingUserAndSshRsa() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
                        "AAAAB3NzaC1yc2EAAAADAQABAAABAQCasJyap4swb4Hk4xOlnF3OmKVwzmv2e053yrtvcUPaxCeboSltOBReuT"
                                + "QxX+kYCgKCdtEwpIvEDXk16T6nCI4tSptAalFgpUWn+JOysCuLuWnwrk6mSKOzEiPYCrB54444mDY6rbBDSRuE/V"
    ...
    @Test(expected = BadRequestException::class)
    fun inValidPublicKeyWillFailWhenTotallyInvalidString() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
                        "ImBAD"))
    }
    ...
    @Test
    fun validSsh2PublicKeyWillNotFail() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
    ...
    @Test(expected = BadRequestException::class)
    fun validSsh2PublicKeyWillNotFailWhenBeginAndEndSsh2Missing() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
                        "Comment: \"2048-bit RSA, converted by ricsi@Richards-MacBook-Pro.local \"\n"
                                + "AAAAB3NzaC1yc2EAAAADAQABAAABAQC0Rfl2G2vDs6yc19RxCqReunFgpYj+ucyLobpTCB\n"
    ...
    @Test(expected = BadRequestException::class)
    fun validSsh2PublicKeyWillFailWhenKeyMissing() {
        rsaPublicKeyValidator!!.validate(
                azureCredential(
                        "Comment: \"2048-bit RSA, converted by ricsi@Richards-MacBook-Pro.local \"\n"
                                + "AAAAB3NzaC1yc2EAAAADAQABAAABAQC0Rfl2G2vDs6yc19RxCqReunFgpYj+ucyLobpTCB\n"
    ...
    private fun azureCredential(publicKey: String): Credential {
