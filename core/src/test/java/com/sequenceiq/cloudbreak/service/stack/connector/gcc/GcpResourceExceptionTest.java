...
public class GcpResourceExceptionTest {
    ...
    public void shouldMessageBeWellFormattedWhenUsingCustomConstructor() throws Exception {
        ...
        //WHEN
        GcpResourceException subject = new GcpResourceException("Error!", ResourceType.GCC_DISK, "disk-name");

        //THEN
        Assert.assertEquals("The error message has not the right format!", "Error!: [ resourceType: GCC_DISK,  resourceName: disk-name ]", subject.getMessage());
    }
    ...
    public void testShouldMessageBeWellFormattedWhenUsingLongConstructor() throws Exception {
        ...
        //WHEN
        GcpResourceException subject = new GcpResourceException("Error!", ResourceType.GCC_DISK, "disk-name", 123L, "create");

        //THEN
        Assert.assertEquals("The error message has not the right format!",
                "Error!: [ resourceType: GCC_DISK,  resourceName: disk-name, stackId: 123, operation: create ]", subject.getMessage());
