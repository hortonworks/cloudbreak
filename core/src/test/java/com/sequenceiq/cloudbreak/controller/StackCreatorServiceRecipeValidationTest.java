...
class StackCreatorServiceRecipeValidationTest {
    ...
    private Answer<Recipe> withNotFoundExceptionIfRecipeNameMatchesOtherwiseGiveRecipe(String recipeNameForMatch) {
        return invocation -> {
            NameOrCrnReader nameOrCrnReader = NameOrCrnReader.create(invocation.getArgument(0));
            if (nameOrCrnReader.getName().equals(recipeNameForMatch)) {
                throw new NotFoundException("Recipe not found!");
            }
            return getRecipeWithName(nameOrCrnReader.getName());
        };
    }
