...
class MigrateZookeeperToKraftInitHandlerTest {
    private static final long STACK_ID = 1L;

    @InjectMocks
    private MigrateZookeeperToKraftInitHandler underTest;

    @Test
    void testDoAcceptShouldReturnStartMigrateZookeeperToKraftValidationEvent() {
        MigrateZookeeperToKraftEvent request = new MigrateZookeeperToKraftEvent(START_MIGRATE_ZOOKEEPER_TO_KRAFT_INIT_EVENT.name(), STACK_ID);
        Selectable result = underTest.doAccept(new HandlerEvent<>(Event.wrap(request)));

        assertEquals(START_MIGRATE_ZOOKEEPER_TO_KRAFT_VALIDATION_EVENT.selector(), result.selector());
    }
}