...
class MigrateZookeeperToKraftUpscaleKraftNodesHandlerTest {
    private static final long STACK_ID = 1L;

    @InjectMocks
    private MigrateZookeeperToKraftUpscaleKraftNodesHandler underTest;

    @Test
    void testDoAcceptShouldReturnStartMigrateZookeeperToKraftEvent() {
        MigrateZookeeperToKraftEvent request = new MigrateZookeeperToKraftEvent(START_MIGRATE_ZOOKEEPER_TO_KRAFT_UPSCALE_KRAFT_NODES_EVENT.name(), STACK_ID);
        Selectable result = underTest.doAccept(new HandlerEvent<>(Event.wrap(request)));

        assertEquals(START_MIGRATE_ZOOKEEPER_TO_KRAFT_EVENT.selector(), result.selector());
    }
}