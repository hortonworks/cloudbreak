...
public class AltusAuthorizationServiceTest {
    ...
    @InjectMocks
    private AltusAuthorizationService underTest;
    ...
    public void testCheckRight() {
        when(umsClient.checkRight(anyString(), anyString(), anyString())).thenReturn(false);
    ...
    public void testGetUserRoles() {
        ...
        when(umsClient.listResourceRoleAssigments(user.getCrn())).thenReturn(Lists.newArrayList(
    ...
    public void testRemoveResourceRolesOfUser() {
        doNothing().when(umsClient).unassignResourceRole(anyString(), anyString(), anyString());
        when(umsClient.listResourceRoleAssigments(anyString())).thenReturn(Lists.newArrayList(
                createResourceRoleAssigment(WORKSPACE_READER_ROLE_CRN, WORKSPACE_CRN),
                createResourceRoleAssigment(WORKSAPCE_WRITER_ROLE_CRN, WORKSPACE_CRN)
        ));
        ...
        verify(umsClient, times(1)).unassignResourceRole(eq(USER_CRN), eq(WORKSPACE_CRN), eq(WORKSPACE_READER_ROLE_CRN));
        verify(umsClient, times(1)).unassignResourceRole(eq(USER_CRN), eq(WORKSPACE_CRN), eq(WORKSAPCE_WRITER_ROLE_CRN));
    }
    ...
    public void testGetUsersOfWorkspace() {
        User user = createUser();
        when(umsClient.listAssigneesOfResource(USER_CRN, WORKSPACE_CRN)).thenReturn(Lists.newArrayList(createResourceAssignee(USER_CRN)));
        ArgumentCaptor<Set<String>> findCaptor = ArgumentCaptor.forClass(Set.class);
