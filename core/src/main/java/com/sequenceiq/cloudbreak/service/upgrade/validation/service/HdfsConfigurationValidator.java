...
public class HdfsConfigurationValidator implements ServiceUpgradeValidator {
    ...
    public void validate(ServiceUpgradeValidationRequest validationRequest) {
        if (validationRequest.rollingUpgradeEnabled() && rollingUpgradeValidationEnabled()) {
            validateHbaseConfigurationForRollingUpgrade(validationRequest.stack());
        } else {
    ...
    private void validateHbaseConfigurationForRollingUpgrade(StackDto stack) {
        ...
        Optional<String> config = connector.getRoleConfigValueByServiceType(stack.getName(), HdfsRoles.GATEWAY, HdfsRoles.HDFS, HDFS_CLIENT_CONFIG_SAFETY_VALVE);
