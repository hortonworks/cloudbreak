...
public class ComponentConfigProviderService {
    ...
    public List<Component> store(List<Component> components) {
        Iterable<Component> savedComponents = componentRepository.saveAll(components);
        savedComponents.forEach(comp -> LOGGER.debug("Component saved: stackId: {}, component: {}", comp.getStack().getId(), comp));
        return components;
    }
