...
public class UpdateNodeCountValidator {
    ...
    public void validateClouderaManagerLimitationForDownscale(Map<String, Integer> adjustmentByHostGroup) {
        for (Map.Entry<String, Integer> hostgroupWithAdjustment : adjustmentByHostGroup.entrySet()) {
            if (hostgroupWithAdjustment.getValue() <= 0) {
                throw new BadRequestException("Cloudera Manager does not support lower node count than 1 for '" + hostgroupWithAdjustment.getKey() + "' " +
                        "group.");
            }
        }
    }
