...
public class AmbariRepositoryVersionService {
    ...
    public void setBaseRepoURL(long clusterId, Orchestrator orchestrator, StackService ambariClient) throws CloudbreakException {
        StackRepoDetails stackRepoDetails = getStackRepoDetails(clusterId, orchestrator);
        if (stackRepoDetails != null) {
            try {
                ...
                if (isNewerOrEqualAmbariApi(AMBARI_VERSION_2_6_0_0, ambariRepoDetails::getVersion)) {
                    ...
                    addVersionDefinitionFileToAmbari(ambariClient, stackRepoDetails);
                }
    ...
    private void addVersionDefinitionFileToAmbari(StackService ambariClient, StackRepoDetails stackRepoDetails) {
        Optional<String> vdfUrl = Optional.ofNullable(stackRepoDetails.getStack().get(CUSTOM_VDF_REPO_KEY));
        if (!vdfUrl.isPresent()) {
            String message = "Couldn't determine any VDF file";
