...
public class WorkspaceService {
    ...
    public Workspace create(User user, Workspace workspace) {
        try {
            return transactionService.required(() -> {
                ...
                altusAuthorizationService.assignResourceRoleToUserInWorkspace(user, workspace, WorkspaceRole.WORKSPACEMANAGER);
    ...
    public Set<Workspace> retrieveForUser(User user) {
        return altusAuthorizationService.getWorkspacesOfCurrentUser(user);
    }
    ...
    public Workspace get(Long id, User user) {
        Optional<Workspace> workspace = altusAuthorizationService.getWorkspacesOfCurrentUser(user).stream()
                .filter(workspaceOfUser -> workspaceOfUser.getId().equals(id))
                .findAny();
        if (!workspace.isPresent()) {
    ...
    public Set<User> removeUsers(String workspaceName, Set<String> userIds, User currentUser) {
        ...
        try {
            return transactionService.required(() -> {
                ...
                altusAuthorizationService.removeResourceRolesOfUserInWorkspace(usersToBeRemoved, workspace);
    ...
    public Set<User> updateUsers(String workspaceName, Set<ChangeWorkspaceUsersV4Request> updateWorkspaceUsersJsons, User currentUser) {
        Workspace workspace = getByNameForUserOrThrowNotFound(workspaceName, currentUser);
        Set<User> usersInWorkspace = altusAuthorizationService.getUsersOfWorkspace(currentUser, workspace);
        verifierService.authorizeWorkspaceManipulation(currentUser, workspace, ResourceAction.MANAGE,
        ...
        try {
            return transactionService.required(() -> {
                ...
                Set<User> removableUsers = usersInWorkspace.stream()
                        ...
                        .collect(Collectors.toSet());
                altusAuthorizationService.removeResourceRolesOfUserInWorkspace(removableUsers, workspace);
    ...
    public Workspace deleteByNameForUser(String workspaceName, User currentUser, Workspace defaultWorkspace) {
        ...
        try {
            return transactionService.required(() -> {
                ...
                altusAuthorizationService.notifyAltusAboutResourceDeletion(currentUser, workspaceForDelete);
    ...
    private void addUserByRequest(Set<ChangeWorkspaceUsersV4Request> changeWorkspaceUsersV4Requests, Workspace workspace, User user) {
        ...
        if (requestForUser.isPresent() && requestForUser.get().getRoles() != null) {
            requestForUser.get().getRoles().stream()
                    .forEach(role -> altusAuthorizationService.assignResourceRoleToUserInWorkspace(user, workspace, role));
        }
    }

    private void updateUser(Set<ChangeWorkspaceUsersV4Request> requestWithRoles, Workspace workspace, User userToBeChange) {
        Set<WorkspaceRole> userCurrentRoles = altusAuthorizationService.getUserRolesInWorkspace(userToBeChange, workspace);
        ...
        if (!userCurrentRoles.containsAll(newRoles) || !newRoles.containsAll(userCurrentRoles)) {
            userCurrentRoles.stream()
                    .forEach(currentRole -> altusAuthorizationService.unassignResourceRoleFromUserInWorkspace(userToBeChange, workspace, currentRole));
            newRoles.stream()
                    .forEach(currentRole -> altusAuthorizationService.assignResourceRoleToUserInWorkspace(userToBeChange, workspace, currentRole));
        }
    }
