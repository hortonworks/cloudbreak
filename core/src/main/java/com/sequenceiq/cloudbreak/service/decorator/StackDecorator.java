...
public class StackDecorator {
    ...
    private void checkSharedServiceStackRequirements(StackV4Request request, User user, Workspace workspace) {
        if (!isSharedServiceRequirementsMeets(request, user, workspace)) {
    ...
    private boolean isSharedServiceRequirementsMeets(StackV4Request request, User user, Workspace workspace) {
    ...
    private boolean hasConfiguredRdsByType(StackV4Request request, User user, Workspace workspace, DatabaseType databaseType) {
        boolean hasConfiguredRds = false;
        if (!request.getCluster().getDatabases().isEmpty()) {
            for (String database : request.getCluster().getDatabases()) {
                if (databaseType.name().equalsIgnoreCase(rdsConfigService.getByNameForWorkspace(database, workspace).getType())) {
