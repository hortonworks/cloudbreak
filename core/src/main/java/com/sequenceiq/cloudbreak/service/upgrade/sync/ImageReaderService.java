...
public class ImageReaderService {
     ...
     */
    Set<ClouderaManagerProduct> getParcels(Set<StatedImage> statedImages, boolean datalake) {
        Set<ClouderaManagerProduct> foundParcelProducts = statedImages.stream()
                .map(ci -> clouderaManagerProductTransformer.transform(ci.getImage(), !datalake))
     ...
     * @param statedImages images to get CM repos
     * @return a set of found CM repos
     */
    Set<ClouderaManagerRepo> getCmRepos(Set<StatedImage> statedImages) {
        Set<ClouderaManagerRepo> foundClouderaManagerRepos = statedImages.stream()
    ...
    private Optional<ClouderaManagerRepo> getCmComponent(StatedImage statedImage) {
        try {
            return imageService.getClouderaManagerRepo(statedImage.getImage());
        } catch (CloudbreakImageCatalogException e) {
            LOGGER.info("Failed to get CM component from stated image {}", statedImage);
