...
@Component
public class ConsulRecipeExecutor {

    private static final String RECIPE_KEY_PREFIX = "consul-watch-plugin/";
    private static final Logger LOGGER = LoggerFactory.getLogger(ConsulRecipeExecutor.class);
    ...
    public void preInstall(Stack stack, Set<HostGroup> hostGroups) throws CloudbreakSecuritySetupException, CloudbreakRecipeSetupException {
    ...
    public void executePreInstall(Stack stack, Set<HostMetadata> hostMetadata) throws CloudbreakSecuritySetupException {
        pluginManager.triggerAndWaitForPlugins(stack, ConsulPluginEvent.PRE_INSTALL, DEFAULT_RECIPE_TIMEOUT, AMBARI_AGENT,
                Collections.emptyList(), getHostnames(hostMetadata));
    }
    ...
    public void configureSssd(Stack stack, Set<HostMetadata> hostMetadata, List<String> sssdPayload) throws CloudbreakException {
        Set<String> hosts = hostMetadata == null ? null : Sets.newHashSet(getHostnames(hostMetadata));
    ...
    private void setupRecipes(Stack stack, Set<HostGroup> hostGroups) throws CloudbreakSecuritySetupException {
        Set<InstanceMetaData> instances = instanceMetadataRepository.findNotTerminatedForStack(stack.getId());
    ...
    private void executePreInstall(Stack stack) throws CloudbreakSecuritySetupException, CloudbreakRecipeSetupException {
        try {
            pluginManager.triggerAndWaitForPlugins(stack, ConsulPluginEvent.PRE_INSTALL, DEFAULT_RECIPE_TIMEOUT, AMBARI_AGENT);
