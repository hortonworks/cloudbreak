    ...
    public Database convert(Stack stack, CloudPlatform cloudPlatform, DatabaseRequest source) {
        ...
        if (source != null) {
            database = DatabaseParameterFallbackUtil.setupDatabaseInitParams(
                    stack, source.getAvailabilityType(), source.getDatabaseEngineVersion(), configureAzureDatabaseIfNeeded(cloudPlatform, source));
        }
        return database;
    }

    private Json configureAzureDatabaseIfNeeded(CloudPlatform cloudPlatform, DatabaseRequest databaseRequest) {
