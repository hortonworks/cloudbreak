...
abstract class AbstractSecurityGroupValidationAction<P extends Payload>
        extends AbstractStackAction<SecurityGroupValidationState, SecurityGroupValidationEvent, StackContext, P> {
    ...
    AbstractSecurityGroupValidationAction(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected StackContext createFlowContext(FlowParameters flowParameters,
        StateContext<SecurityGroupValidationState, SecurityGroupValidationEvent> stateContext, P payload) {
    ...
    protected void sendValidationFailure(StackContext context, Long resourceId, String errorReason) {
        sendEvent(context, SECURITY_GROUP_VALIDATION_FAIL_EVENT.event(),
                new StackFailureEvent(resourceId, new CloudbreakServiceException(errorReason)));
    }
}
