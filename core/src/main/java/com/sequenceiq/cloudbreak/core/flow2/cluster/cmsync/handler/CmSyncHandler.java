    ...
    protected Selectable doAccept(HandlerEvent<CmSyncRequest> event) {
        ...
        try {
            ...
            if (!cmSyncOperationSummary.hasSucceeded()) {
                throw new RuntimeException(String.format("Syncing CM version and installed parcels encountered failures. Details: %s",
                        cmSyncOperationSummary.getMessage()));
            }
            return new CmSyncResult(request, cmSyncOperationSummary);
        } catch (Exception e) {
            LOGGER.warn("Error during syncing CM version to DB, syncing skipped.", e);
            return new CmSyncResult("Error during syncing from CM", e, request);
        }
    }
