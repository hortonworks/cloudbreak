    ...
    public Action<?, ?> finishCmSync() {
        return new AbstractCmSyncAction<>(CmSyncResult.class) {
            ...
            protected Selectable createRequest(CmSyncContext context) {
                return new StackEvent(CmSyncEvent.FINALIZED_EVENT.event(), context.getStack().getId());
            }
    ...
    public Action<?, ?> cmSyncFailedAction() {
        return new AbstractStackFailureAction<CmSyncState, CmSyncEvent>() {
            ...
            protected void doExecute(StackFailureContext context, StackFailureEvent payload, Map<Object, Object> variables) {
                LOGGER.debug("Error during executing syncing Cloudera Manager and parcels versions from CM.", payload.getException());
                sendEvent(context);
            }

            @Override
            protected Selectable createRequest(StackFailureContext context) {
                return new StackEvent(CmSyncEvent.FAIL_HANDLED_EVENT.event(), context.getStackView().getId());
            }
        };
    }
}
