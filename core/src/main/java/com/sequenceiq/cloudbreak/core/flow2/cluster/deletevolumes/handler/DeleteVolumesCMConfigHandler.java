...
public class DeleteVolumesCMConfigHandler extends ExceptionCatcherEventHandler<DeleteVolumesCMConfigEvent> {
    ...
    public Selectable doAccept(HandlerEvent<DeleteVolumesCMConfigEvent> deleteVolumesCMConfigEvent) {
        LOGGER.debug("Staring DeleteVolumesCMConfigHandler with event: {}", deleteVolumesCMConfigEvent);
        ...
        try {
            ...
            if (checkConfigChangeRequired(hostTemplateComponents)) {
                ...
                configUpdateUtilService.updateCMConfigsForComputeAndStartServices(stackDto, hostTemplateServiceComponents, getInstanceStorageCount(stackDto,
                        requestGroup), 0, hostTemplateRoleGroupNames, TemporaryStorage.EPHEMERAL_VOLUMES_ONLY);
            }
    ...
    private int getInstanceStorageCount(StackDto stackDto, String requestGroup) {
        int instanceStorageCount = 0;
        Optional<InstanceGroupView> optionalGroup = stackDto.getInstanceGroupViews().stream().filter(group -> group.getGroupName().equals(requestGroup))
                .findFirst();
        if (optionalGroup.isPresent()) {
            InstanceGroupView instanceGroup = optionalGroup.get();
            instanceStorageCount = instanceGroup.getTemplate().getInstanceStorageCount();
        }
