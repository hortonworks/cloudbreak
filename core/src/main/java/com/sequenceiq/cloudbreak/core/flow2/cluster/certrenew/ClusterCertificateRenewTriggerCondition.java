...
public class ClusterCertificateRenewTriggerCondition implements FlowTriggerCondition {
    ...
    public boolean isFlowTriggerable(Long stackId) {
        ...
        if (!resourcesIsInTriggerableState) {
            LOGGER.info("Certificate renewal could not be triggered, because the cluster's state is not available.");
        }
        ...
        if (!certRenewalIsTriggerable) {
            LOGGER.info("Certificate renewal could not be triggered, because the generation feature is disabled or certificate that"
                    + " is generated by PEM service does not belong to the specified stack.");
        }
        return resourcesIsInTriggerableState && certRenewalIsTriggerable;
    }
}
