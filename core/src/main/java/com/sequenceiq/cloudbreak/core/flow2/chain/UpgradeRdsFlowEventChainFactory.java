...
public class UpgradeRdsFlowEventChainFactory implements FlowEventChainFactory<RdsUpgradeChainTriggerEvent> {
    ...
    public FlowTriggerEventQueue createFlowTriggerEventQueue(RdsUpgradeChainTriggerEvent event) {
        ...
        flowEventChain.add(new ValidateRdsUpgradeTriggerRequest(ValidateRdsUpgradeEvent.VALIDATE_RDS_UPGRADE_EVENT.event(), event.getResourceId()));
        flowEventChain.add(new UpgradeRdsTriggerRequest(UpgradeRdsEvent.UPGRADE_RDS_EVENT.event(), event.getResourceId(), event.getVersion()));
