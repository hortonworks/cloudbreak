...
abstract class AbstractCoreEnableSeLinuxAction<P extends Payload>
        extends AbstractAction<CoreEnableSeLinuxState, CoreEnableSeLinuxStateSelectors, StackContext, P> {
    ...
    protected StackContext createFlowContext(FlowParameters flowParameters,
            StateContext<CoreEnableSeLinuxState, CoreEnableSeLinuxStateSelectors> stateContext, P payload) {
    ...
    protected Object getFailurePayload(P payload, Optional<StackContext> flowContext, Exception ex) {
        return new CoreEnableSeLinuxFailedEvent(payload.getResourceId(), "Unexpected error during action", ex);
    }
}
