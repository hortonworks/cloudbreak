...
abstract class AbstractCoreEnableSeLinuxAction<P extends ResourceCrnPayload>
        extends AbstractAction<CoreEnableSeLinuxState, CoreEnableSeLinuxStateSelectors, CommonContext, P> {

    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractCoreEnableSeLinuxAction.class);

    protected AbstractCoreEnableSeLinuxAction(Class<P> payloadClass) {
        super(payloadClass);
    }

    @Override
    protected CommonContext createFlowContext(FlowParameters flowParameters,
            StateContext<CoreEnableSeLinuxState, CoreEnableSeLinuxStateSelectors> stateContext,
            P payload) {
