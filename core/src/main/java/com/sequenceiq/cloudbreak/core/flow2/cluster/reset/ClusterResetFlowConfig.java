...
import static com.sequenceiq.cloudbreak.core.flow2.cluster.reset.ClusterResetEvent.CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_EVENT;
import static com.sequenceiq.cloudbreak.core.flow2.cluster.reset.ClusterResetEvent.CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_FAILURE_EVENT;
...
import static com.sequenceiq.cloudbreak.core.flow2.cluster.reset.ClusterResetState.CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_STATE;
...
@Component
public class ClusterResetFlowConfig extends StackStatusFinalizerAbstractFlowConfig<ClusterResetState, ClusterResetEvent> {
    private static final List<Transition<ClusterResetState, ClusterResetEvent>> TRANSITIONS =
            new Builder<ClusterResetState, ClusterResetEvent>()
                    .from(INIT_STATE)
                    .to(CLUSTER_RESET_STATE)
                    .event(CLUSTER_RESET_EVENT)
                    .noFailureEvent()
                    .from(CLUSTER_RESET_STATE)
                    .to(CLUSTER_RESET_FINISHED_STATE)
                    .event(CLUSTER_RESET_FINISHED_EVENT)
                    .failureEvent(CLUSTER_RESET_FINISHED_FAILURE_EVENT)
                    .from(CLUSTER_RESET_FINISHED_STATE)
                    .to(CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_STATE)
                    .event(CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_EVENT)
                    .failureEvent(CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_FAILURE_EVENT)
                    .from(CLUSTER_RESET_START_CLUSTER_MANAGER_FINISHED_STATE)
                    .to(FINAL_STATE)
    ...
    @Override
    public FlowEdgeConfig<ClusterResetState, ClusterResetEvent> getEdgeConfig() {
        return EDGE_CONFIG;
    }
