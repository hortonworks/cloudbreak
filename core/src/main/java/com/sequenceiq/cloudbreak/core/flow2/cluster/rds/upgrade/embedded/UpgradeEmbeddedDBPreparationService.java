...
//    @Inject
//    private UpgradeRdsOrchestratorService upgradeRdsOrchestratorService;
//
//    @Inject
//    private RdsResourceTerminationListener rdsResourceTerminationListener;
//
//    @Inject
//    private RdsV2AgentTerminationListener rdsV2AgentTerminationListener;
...
//    private HealthCheckService healthCheckService;
    ...
    void stopServicesState(Long stackId) {
        setStatusAndNotify(stackId, "Stopping Runtime Services and Cloudera Manager", ResourceEvent.CLUSTER_RDS_UPGRADE_STOP_SERVICES);
    }

    void backupRdsState(Long stackId) {
        setStatusAndNotify(stackId, "Creating data backup, it might take a while", ResourceEvent.CLUSTER_RDS_UPGRADE_BACKUP_DATA);
    }

    void upgradeRdsState(Long stackId) {
        setStatusAndNotify(stackId, "Upgrading database server", ResourceEvent.CLUSTER_RDS_UPGRADE_DBSERVER_UPGRADE);
    }

    void restoreRdsState(Long stackId) {
        setStatusAndNotify(stackId, "Restoring data from the backup, it might take a while", ResourceEvent.CLUSTER_RDS_UPGRADE_RESTORE_DATA);
    }

    void startServicesState(Long stackId) {
        setStatusAndNotify(stackId, "Starting back Cloudera Manager and Runtime Services", ResourceEvent.CLUSTER_RDS_UPGRADE_START_SERVICES);
    }

    void setStatusAndNotify(Long stackId, String statusReason, ResourceEvent resourceEvent) {
