...
public class MigrateZookeeperToKraftValidationHandler extends ExceptionCatcherEventHandler<MigrateZookeeperToKraftEvent> {
    ...
    protected Selectable doAccept(HandlerEvent<MigrateZookeeperToKraftEvent> event) {
        ...
        try {
            ...
            if (KraftMigrationStatus.BROKERS_IN_KRAFT.equals(kraftMigrationStatus)
                ...
                return new MigrateZookeeperToKraftEvent(FINISH_MIGRATE_ZOOKEEPER_TO_KRAFT_EVENT.name(), stackId,
                        event.getData().isStaleConfigsOnly(), event.getData().isKraftHostGroupPresent());
            }
        ...
        return new MigrateZookeeperToKraftEvent(START_RESTART_KAFKA_ROLES_EVENT.name(), stackId,
                event.getData().isStaleConfigsOnly(), event.getData().isKraftHostGroupPresent());
    }
