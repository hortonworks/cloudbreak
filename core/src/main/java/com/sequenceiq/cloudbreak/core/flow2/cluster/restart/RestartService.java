...
public class RestartService {
    ...
    public void startInstanceRestart(RestartContext context) {
        stackUpdater.updateStackStatus(context.getStack().getId(), DetailedStackStatus.RESTART_INSTANCES_IN_PROGRESS,
                "Restarting Instances Started");
    ...
    public void instanceRestartFinished(RestartContext context, List<String> failedToRestartInstanceIds, List<String> successOnRestartInstanceIds) {
        stackUpdater.updateStackStatus(context.getStack().getId(), DetailedStackStatus.RESTART_INSTANCES_COMPLETED,
                "Restarted Instances");
    ...
    public void allInstanceRestartFailed(RestartContext context, Exception errorDetails) {
        ...
        stackUpdater.updateStackStatus(context.getStack().getId(), DetailedStackStatus.RESTART_INSTANCES_FAILED,
                String.format("Instances(s) could not be Restarted: %s", errorDetails));
