    ...
    public Action<?, ?> failedAction() {
        return new AbstractStackFailureAction<DistroXDiskUpdateState, DistroXDiskUpdateStateSelectors>() {
            ...
            protected void doExecute(StackFailureContext context, StackFailureEvent payload, Map<Object, Object> variables) {
                DistroXDiskUpdateFailedEvent event = (DistroXDiskUpdateFailedEvent) payload;
                Long stackId = event.getResourceId();
                DistroXDiskUpdateEvent request = event.getDiskUpdateEvent();
                if (CloudPlatform.GCP.name().equals(request.getCloudPlatform()) && request.getVolumeType() != null) {
                    stackUpdater.updateStackStatus(stackId, DetailedStackStatus.STOPPED, "Failed Updating volumes");
                } else {
