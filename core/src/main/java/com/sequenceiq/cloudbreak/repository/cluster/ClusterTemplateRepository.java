...
public interface ClusterTemplateRepository extends WorkspaceResourceRepository<ClusterTemplate, Long> {
    ...
    @Query("SELECT NEW ClusterTemplate(c.templateContent, c.name, c.status, c.workspace) FROM ClusterTemplate c WHERE c.workspace.id= :workspaceId AND c.status <> 'DEFAULT_DELETED'")
    @CheckPermissionsByReturnValue
