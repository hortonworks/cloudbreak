...
@Service
public class DiskSyncJobService implements JobSchedulerService {

    private static final Logger LOGGER = LoggerFactory.getLogger(DiskSyncJobService.class);

    private static final String JOB_GROUP = "disk-sync-jobs";

    private static final String TRIGGER_GROUP = "disk-sync-triggers";
    ...
    @Inject
    private DiskSyncConfig properties;
    ...
    public void schedule(StackView stack) {
        if (shouldSync(stack)) {
            scheduleJob(new DiskSyncJobAdapter(stack.getId(), applicationContext));
        }
    }

    public void schedule(DiskSyncJobAdapter resource) {
    ...
    private boolean shouldSync(String provider) {
        if (!properties.isDiskSyncEnabled()) {
            return false;
        }
    ...
    private void scheduleJob(DiskSyncJobAdapter resource) {
    ...
    private JobDetail buildJobDetail(DiskSyncJobAdapter resource) {
