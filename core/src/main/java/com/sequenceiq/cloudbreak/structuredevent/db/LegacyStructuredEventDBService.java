...
public class LegacyStructuredEventDBService extends AbstractWorkspaceAwareResourceService<StructuredEventEntity> implements LegacyStructuredEventService {
    ...
    public void deleteEntriesByResourceIdsOlderThanThreeMonths(Long resourceIds) {
        repository.deleteRecordsOlderThan(resourceIds, getTimestampThatThreeMonthsFromNow());
    }

    public void deleteEntriesForTenantThatIsOlderThanThreeMonths(String tenantName) {
        repository.deleteByResourceCrnLikeAndTimestampIsLessThan(decorateTenantForLikelinessSearch(tenantName), getTimestampThatThreeMonthsFromNow());
    }
    ...
    private Long getTimestampThatThreeMonthsFromNow() {
        return Instant.now().getEpochSecond() * MILLISEC_MULTIPLIER - THREE_MONTHS;
    }

    private String decorateTenantForLikelinessSearch(String content) {
        return "%:" + content + ":%";
    }
