...
@Component
public class LegacyCoreStructuredEventDBService extends AbstractWorkspaceAwareResourceService<StructuredEventEntity> implements StructuredEventService {

    private static final Logger LOGGER = LoggerFactory.getLogger(LegacyCoreStructuredEventDBService.class);
    ...
    @Override
    public StructuredEventEntity create(StructuredEventEntity resource, A workspace, User user) {
    ...
    @Override
    public LegacyStructuredEventRepository<StructuredEventEntity, Long> repository() {
    ...
    private Stack getStackIfAvailable(Long workspaceId, String name, StackType stackType) {
        return Optional.ofNullable(stackService.getByNameInWorkspace(name, workspaceId))
                .orElseThrow(NotFoundException.notFound(stackType.getResourceType(), name));
    }
}
