...
public class V2StackEventRestUrlParser extends RestUrlParser {
    ...
    private static final Pattern ANTI_PATTERN = Pattern.compile(".*(scaling|start|stop|sync|reinstall).*");

    private static final Pattern PATTERN = Pattern.compile("v2/stacks/(?!account|user)([^/\\d]+)/([a-z|-]+)");
