...
public class ClusterManagerUpgradeHandler extends ExceptionCatcherEventHandler<ClusterManagerUpgradeRequest> {
    ...
    protected Selectable doAccept(HandlerEvent<ClusterManagerUpgradeRequest> event) {
        ...
        try {
            clusterManagerUpgradeManagementService.upgradeClusterManager(request);
            return new ClusterManagerUpgradeSuccess(request.getResourceId(), request.getUpgradeCandidateProducts(), request.getOriginalOsType());
        } catch (Exception e) {
