...
public class DatalakeRecoverySetupNewInstancesHandler extends ExceptionCatcherEventHandler<DatalakeRecoverySetupNewInstancesRequest> {
    ...
    protected Selectable doAccept(HandlerEvent<DatalakeRecoverySetupNewInstancesRequest> event) {
        ...
        LOGGER.debug("Re-launching instances for stack {}", stackId);
        try {
            Stack stack = stackService.getByIdWithClusterInTransaction(stackId);
            setupInstances(stack);
            clusterService.updateClusterStatusByStackId(stackId, Status.REQUESTED);

            result = new DatalakeRecoverySetupNewInstancesSuccess(stackId);
        } catch (Exception e) {
            LOGGER.error("Relaunching instances for stack failed", e);
            result = new DatalakeRecoverySetupNewInstancesFailedEvent(stackId, e, DetailedStackStatus.CLUSTER_RECOVERY_FAILED);
        }
        return result;
    }

    private void setupInstances(Stack stack) {
