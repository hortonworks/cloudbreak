...
public class UmsAuthorizationService {
    ...
    public void assignResourceRoleToUserInWorkspace(User user, Workspace workspace, WorkspaceRole role) {
        if (umsClient.isUmsUsable(user.getCrn())) {
            umsClient.assignResourceRole(user.getCrn(), workspace.getResourceCrn(), role.getCrn(user.getCrn()), getRequestId());
        }
    }

    public void unassignResourceRoleFromUserInWorkspace(User user, Workspace workspace, WorkspaceRole role) {
        if (umsClient.isUmsUsable(user.getCrn())) {
            umsClient.unassignResourceRole(user.getCrn(), workspace.getResourceCrn(), role.getCrn(user.getCrn()), getRequestId());
        }
    }
    ...
    public void checkRightOfUserForResource(User user, Workspace workspace, WorkspaceResource resource, ResourceAction action, String unauthorizedMessage) {
        if (umsClient.isUmsUsable(user.getCrn())
                && !umsClient.checkRight(user.getCrn(), WorkspaceRightUtils.getRight(resource, action), workspace.getResourceCrn(), getRequestId())) {
    ...
    public boolean hasRightOfUserForResource(User user, Workspace workspace, WorkspaceResource resource, ResourceAction action) {
        if (umsClient.isUmsUsable(user.getCrn())) {
            return umsClient.checkRight(user.getCrn(), WorkspaceRightUtils.getRight(resource, action), workspace.getResourceCrn(), getRequestId());
        }
        return true;
    }
    ...
    public Set<WorkspaceRole> getUserRolesInWorkspace(User user, Workspace workspace) {
        if (umsClient.isUmsUsable(user.getCrn())) {
            return umsClient.listResourceRoleAssigments(user.getCrn(), getRequestId()).stream()
    ...
    public void notifyAltusAboutResourceDeletion(User currentUser, Workspace workspace) {
        if (umsClient.isUmsUsable(currentUser.getCrn())) {
            umsClient.notifyResourceDeleted(currentUser.getCrn(), workspace.getResourceCrn(), getRequestId());
        }
    }

    public Set<User> getUsersOfWorkspace(User currentUser, Workspace workspace) {
        if (umsClient.isUmsUsable(currentUser.getCrn())) {
            Set<String> userIds = umsClient.listAssigneesOfResource(currentUser.getCrn(), workspace.getResourceCrn(), getRequestId()).stream()
    ...
    public Set<Workspace> getWorkspacesOfCurrentUser(User currentUser) {
        if (umsClient.isUmsUsable(currentUser.getCrn())) {
            Set<String> workspaceCrns = umsClient.listResourceRoleAssigments(currentUser.getCrn(), getRequestId()).stream()
        ...
        return Sets.newHashSet(workspaceRepository.getByName(Crn.fromString(currentUser.getCrn()).getAccountId(), currentUser.getTenant()));
    }
