...
function setupAtlasServer() {
  if [ -d "/usr/hdp/current/atlas-server" ]; then
    ...
    curl -iv -u $AMBARI_USER:$AMBARI_PASSWORD -H "X-Requested-By: ambari" -X POST -d '{"RequestInfo":{"command":"RESTART","context":"Restart all components required ATLAS","operation_level":{"level":"SERVICE","cluster_name":"$CLUSTER_NAME","service_name":"ATLAS"}},"Requests/resource_filters":[{"hosts_predicate":"HostRoles/stale_configs=false&HostRoles/cluster_name=$CLUSTER_NAME"}]}' http://$(hostname -f):8080/api/v1/clusters/$CLUSTER_NAME/requests
  fi
}
