...
add_dns_record:
  cmd.run:
    - name: /opt/salt/scripts/add_dns_record.sh > >(tee -a /var/log/dnsrecord-add.log) 2> >(tee -a /var/log/dnsrecord-add.log >&2) && echo $(date +%Y-%m-%d:%H:%M:%S) >> /var/log/dnsrecord-add-executed
    - runas: root
    - failhard: True
