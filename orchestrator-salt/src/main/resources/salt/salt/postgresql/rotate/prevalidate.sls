...
prevalidate-db-secrets-remote:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets_remote.sh prevalidate' 2>&1 | tee -a /var/log/postgresql_rotate_prevalidate.log && exit ${PIPESTATUS[0]}
    - require:
...
prevalidate-db-secrets:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets.sh prevalidate' 2>&1 | tee -a /var/log/postgresql_rotate_prevalidate.log && exit ${PIPESTATUS[0]}
    - require:
