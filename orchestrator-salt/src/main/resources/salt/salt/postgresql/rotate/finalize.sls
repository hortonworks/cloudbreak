...
finalize-db-secrets-remote:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets_remote.sh finalize' 2>&1 | tee -a /var/log/postgresql_rotate_finalize.log && exit ${PIPESTATUS[0]}
    - require:
...
finalize-db-secrets:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets.sh finalize' 2>&1 | tee -a /var/log/postgresql_rotate_finalize.log && exit ${PIPESTATUS[0]}
    - require:
