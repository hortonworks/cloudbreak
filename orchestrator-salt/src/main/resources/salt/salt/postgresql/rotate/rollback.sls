...
rollback-db-secrets-remote:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets_remote.sh rollback 2>&1 | tee -a /var/log/postgresql_rotate_rollback.log && exit ${PIPESTATUS[0]}'
    - require:
...
rollback-db-secrets:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate/rotate_db_secrets.sh rollback 2>&1 | tee -a /var/log/postgresql_rotate_rollback.log && exit ${PIPESTATUS[0]}'
    - require:
