...
postvalidate-db-secrets-remote:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate_db_secrets_remote.sh postvalidate' 2>&1 | tee -a /var/log/postgresql_rotate_postvalidate.log && exit ${PIPESTATUS[0]}
    - require:
...
postvalidate-db-secrets:
  cmd.run:
    - name: runuser -l postgres -s /bin/bash -c '/opt/salt/scripts/rotate_db_secrets.sh postvalidate' 2>&1 | tee -a /var/log/postgresql_rotate_postvalidate.log && exit ${PIPESTATUS[0]}
    - require:
