    ...
    if [[ '{{ ambari_database.ambariVendor }}' = 'embedded' ]]; then
        ...
        if [[ -f /var/lib/ambari-server/resources/Ambari-DDL-{{ ambari_database.fancyName }}-CREATE.sql ]]; then
          ...
          if [[ '{{ ambari_database.ambariVendor }}' = 'mysql' ]]; then
            ...
            cp -f $(find /usr/share/java -name "mysql-connector*.jar" | tail -n1) /usr/share/java/mysql-connector.jar ##copy the jbdc driver jar to /usr/share/java folder because Ambari server requires it in that directory
