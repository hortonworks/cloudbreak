...
import com.sequenceiq.cloudbreak.service.ExecutionMeter;
...
public class SaltConnector implements Closeable {
    ...
    public GenericResponse health() {
        try (ExecutionMeter executionMeter = ExecutionMeter.start("saltboot-health", LOGGER)) {
    ...
    public GenericResponses pillar(Iterable<String> targets, Pillar pillar) {
        try (ExecutionMeter executionMeter = ExecutionMeter.start("saltboot-pillar-distribute", LOGGER)) {
    ...
    public GenericResponses action(SaltAction saltAction) {
        try (ExecutionMeter executionMeter = ExecutionMeter.start("saltboot-action-distribute", LOGGER)) {
    ...
    public <T> T run(Target<String> target, String fun, SaltClientType clientType, Class<T> clazz, String... arg) {
        try (ExecutionMeter executionMeter = ExecutionMeter.start(fun, LOGGER)) {
            Form form = new Form();
            form = addAuth(form)
                    .param("fun", fun)
    ...
    public GenericResponses upload(Iterable<String> targets, String path, String fileName, byte[] content) throws IOException {
        try (ExecutionMeter executionMeter = ExecutionMeter.start("saltboot-file-distribute", LOGGER)) {
    ...
    public Map<String, String> members(List<String> privateIps) throws CloudbreakOrchestratorFailedException {
        try (ExecutionMeter executionMeter = ExecutionMeter.start("saltboot-hostname-distribute", LOGGER)) {
