        ...
        if (!targets.isEmpty()) {
            ...
            for (GenericResponse genericResponse : responses.getResponses()) {
                if (genericResponse.getStatusCode() != HttpStatus.CREATED.value()) {
                    ...
                    failureDetails.add(String.format("%s (status code: %d, error: %s)",
                            address, genericResponse.getStatusCode(), genericResponse.getErrorText()));
                }
            }
            targets = failedTargets;

            if (!targets.isEmpty()) {
                LOGGER.info("Missing nodes for file upload: {}", failureDetails);
                throw new CloudbreakOrchestratorFailedException(
                        "There are missing nodes for file upload: " + targets + ". Details: " + failureDetails);
            }
        }
